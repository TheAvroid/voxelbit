...
//
// WHAT CLUSTERS ARE FOR. v2 builds one bottom-level acceleration structure per
...
// VK_NV_cluster_acceleration_structure at revision 4, and v2 already runs on
...
// it enabled is for v2 to CREATE THE VULKAN DEVICE ITSELF and hand it over
...
class Clusters {
  public:
    ...
    //
    // v2's mesher makes that problem disappear. VoxMesh::addQuad pushes FOUR
    ...
    ClusterBlas build(Falcor::RenderContext *ctx, Falcor::Buffer *verts, uint32_t quadCount) {
        ...
        auto argBuf = upload(args.data(), args.size() * sizeof(args[0]), "v2::clasArgs");
        ...
        // PER-OBJECT maximum in this mode, so v2 can lay the CLAS out itself at
        ...
        // v2 chooses where each CLAS goes, so the addresses are known here and
        ...
        auto clasAddrs = upload(clasVAs.data(), clasVAs.size() * 8, "v2::clasAddrs");
        ...
        auto blasArgBuf = upload(&blasArg, sizeof(blasArg), "v2::blasArgs");
        ...
        // knowable without a readback: v2 says where the result goes, so the
        ...
        auto blasAddr = upload(&blasVa, sizeof(blasVa), "v2::blasAddr");
  ...
  private:
    ...
    bool ensureSharedIndices() {
        ...
        sharedIdx_->setName("v2::clusterIndices");
        return true;
    }
