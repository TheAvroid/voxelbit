...
class Bullets {
  public:
    ...
    // WHERE THE ROUND WAS AND WHERE IT IS GOING -- see Arrows::LifeF, which
    // this mirrors and which explains why a point is not enough. A bullet steps
    // at kBulletStepM and is faster than a shaft, so it is the worse case of
    // the two.
