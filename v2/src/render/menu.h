...
class SettingsMenu {
  public:
    ...
    void draw(TextPanel &panel, const MenuStatus &st) {
        ...
        panel.compose(214, cornerRadius(panel));
    }

    // Rounded off in proportion to the text, so the corner looks the same at
    // every font size the viewer picks. Capped at both ends: below about ten
    ...
    // lozenge.
    static int cornerRadius(const TextPanel &p) {
        return maxi(10, mini(22, p.lineHeight()));
    }
  ...
  private:
    ...
    std::string value(int i) const {
        ...
        switch (i) {
            ...
            case 9:
                std::snprintf(b, sizeof(b), "%-9d %s", t_.samplesPerFrame ? *t_.samplesPerFrame : 1,
                              "traced per displayed frame -- noise falls as sqrt(n)");
                break;
            case 10: {
                const unsigned n = t_.liveMaxAccum ? *t_.liveMaxAccum : 0u;
                if (t_.constantGrain && *t_.constantGrain)
                    std::snprintf(b, sizeof(b), "%-9s never settles -- %u samples still AND moving",
                                  "on", n);
                else if (n == 0u)
                    std::snprintf(b, sizeof(b), "%-9s %s", "off",
                                  "converges when still, clock stopped");
                else
    ...
    void adjust(int dir) {
        switch (row_) {
            ...
            case 9:
                if (t_.samplesPerFrame) {
                    *t_.samplesPerFrame = mini(64, maxi(1, *t_.samplesPerFrame + dir));
                    invalidated = true;
                }
                return;
            case 10:
