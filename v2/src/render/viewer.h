...
struct ViewerOptions {
    ...
    //
    // This is the headroom knob, and it exists because the still image was
    // never the problem -- a still camera piles up samples until it converges,
    // and a moving one starts again from one sample every frame. At 1280x720
    // this engine renders a sample in about 1.4 ms, so at 60 fps there is room
    // for a dozen of them and at 120 fps for six. Spending that budget is the
    // cheapest noise reduction available: it is the same estimator, so noise
    // falls as sqrt(n) with no blur, no history, no artefacts and nothing to
    // go wrong on a disocclusion.
    // ONE by default, because this is a frame-rate decision and it is not
    // mine to make. Measured at the size this engine actually opens at,
    // 2674x1393 traced: 110 fps at one sample, 61 at two, 38 at three, 36 at
    // four. Noise falls as sqrt(n), so two samples is 1.4x cleaner for half
    // the frame rate -- a trade worth offering and not worth imposing. The
    // menu row moves it live with the fps counter two lines above it.
...
class Viewer {
  public:
    ...
    int run(Vec3 startPos, float startYaw, float startPitch, float fovDeg) {
        ...
        player_.placeOnGround(walkWorld(), startPos.x, startPos.z);
        ...
        // A background launch must not steal the foreground, so it is created
        // hidden and shown iconified rather than shown and then minimised --
        // the latter flashes a window across whatever the user is looking at.
        glfwWindowHint(GLFW_FOCUS_ON_SHOW, opt_.background ? GLFW_FALSE : GLFW_TRUE);
        if (opt_.background) {
        ...
        window_ = glfwCreateWindow(opt_.width, opt_.height,
                                   opt_.background ? "v2 [background] -- pine forest (OptiX)"
                                                   : "v2 -- pine forest (OptiX)", nullptr,
                                   nullptr);
        if (!window_) {
        ...
        if (opt_.background) {
            // Iconify BEFORE showing, so it goes straight to the taskbar.
        ...
        // AFTER the restore, not before: the size dance above deliberately
        // un-minimises the window to defeat Windows restoring a remembered
        // placement, which would otherwise undo an earlier iconify. So a
        // background run is put back in the taskbar here, once the size it was
        // asked for has actually been applied.
        if (opt_.background) glfwIconifyWindow(window_);
        ...
        while (!glfwWindowShouldClose(window_)) {
            ...
            glfwGetFramebufferSize(window_, &fbW_, &fbH_);
            // A minimised window reports 0x0, and idling on that is right for a
            // person's game -- but a background instance is minimised ON
            // PURPOSE and exists to be measured, so it keeps rendering at the
            // size it was asked for instead of stopping.
            if (opt_.background && (fbW_ <= 0 || fbH_ <= 0)) {
                fbW_ = maxi(16, opt_.width);
                fbH_ = maxi(16, opt_.height);
            }
            if (fbW_ <= 0 || fbH_ <= 0) {
            ...
            if (fpsAccum >= 0.5) {
                ...
                std::snprintf(title, sizeof(title),
                              "v2%s -- pine forest  |  %dx%d  %u spp  %.1f fps%s",
                              opt_.background ? " [background]" : "",
  ...
  private:
    ...
    unsigned lastMaxAccum_ = 0u;  // what the film window worked out to, for the menu
    ViewerOptions opt_;
    ...
    // reached at 17 m/s.
    WalkWorld walkWorld() {
        scene_.collidersNear(player_.pos, 6.0f, &solids_);
        WalkWorld w;
        w.terrain = &scene_.terrain;
    ...
    int shotIndex_ = 0;
    float msBlit_ = 0.0f;
    static constexpr int kHudX = 28, kHudY = 20;  // fps readout, framebuffer px
    ...
    // Realise the settings panel at the current window size. Two steps,
    // because the panel's size is quoted in CHARACTERS and the character width
    // is only known once a font has been realised.
    void sizePanel() {
        ...
        if (fontPx == panelFont_) return;
        panel_.resize(16, 16, fontPx);
        panel_.resize(menu_.panelWidth(panel_), menu_.panelHeight(panel_), fontPx);
        panelFont_ = fontPx;
    }

    // Where the settings panel sits: the middle of the window, which is where
    // the eyes already are -- it used to be tucked under the fps readout in the
    // top-left corner, which is the one part of the screen you are not looking
    // at while flying. The readout stays where it was; it is read WHILE moving,
    // and a number that follows the crosshair around is a number in the way.
    //
    // One function, used by the blit AND by the cursor mapping. The click has
    // to be tested against the rectangle the panel was actually drawn in, and
    // the moment those are two separate expressions they disagree on the first
    // resize.
    void menuOrigin(int *x, int *y) const {
        *x = maxi(0, (fbW_ - panel_.width()) / 2);
        *y = maxi(0, (fbH_ - panel_.height()) / 2);
    }
    ...
    void renderFrame() {
        ...
        // and a different amount of light. With constant grain that would
        // leave a visible step between standing and walking even with the
        // sample counts matched, so the path length is pinned too. Bounces are
        // nearly free here (three to eight costs about three percent), so this
        // costs very little of what the trade was buying.
        if (moving_ && !opt_.constantGrain)
            cfg.maxDepth = mini(base_.maxDepth, opt_.movingDepth);
        ...
        // IS the noise floor.
        //
        // Constant grain pins N to the number of samples a MOVING frame gets.
        // Moving throws the film away every frame, so a walking camera sees
        // exactly samplesPerFrame samples; capping a standing one at the same
        // number makes the two identical and the picture never settles.
        ...
        if (opt_.constantGrain) {
            ...
            // one -- "slightly less noise", exactly as reported.
            //
            // The film is instead thrown away every displayed frame below, so
            // still and moving are both the mean of the same N fresh samples
            ...
            cfg.maxAccum = (unsigned)clampf(n, 32.0f, 8192.0f);
        }
        lastMaxAccum_ = cfg.maxAccum;
        ...
        const CameraGPU gcam = cam.gpu(rend_.renderWidth(), rend_.renderHeight());
        // Several samples per displayed frame rather than one. They land in
        // the same accumulator, so this is exactly the still-camera behaviour
        ...
        // Skipping a resolve is only ever worth it when the skipped frames go
        // into ACCUMULATING -- the picture converges faster for being redrawn
        // less often. Under constant grain nothing accumulates: the film is
        // thrown away every frame, so a skipped resolve traces samples and
        // discards them, and standing still would update at a third the rate
        // of walking for no benefit. So it always resolves, which is also the
        // last thing that still differed between moving and standing.
        const bool resolveNow = opt_.constantGrain || moving_ || rend_.samples() <= 8 ||
                                (rend_.samples() % 3) == 0;
    ...
    // -----------------------------------------------------------------------
    // THE CROSSHAIR -- v1's, ported: a white plus in the middle of the screen,
    // XORed into whatever is behind it. Twice v1's size (user), which on this
    // engine's default window is 32 px across and 4 px thick.
    //
    // v1 builds it from bars with `mix-blend-mode: difference`, and the blend
    // is the point rather than a flourish. White differenced against the
    // backdrop is `1 - backdrop`, so the cross INVERTS the frame under it and
    // cannot go blind: a plain white crosshair vanishes on snow, on a bright
    // sky and on a white flower, which in this world is most of what you would
    // be aiming at. An inverted one is visible on all three.
    //
    // The fixed pipeline has no difference blend, and glBlendEquation needs an
    // extension loader this file deliberately does not have. But on 8-bit
    // channels `1 - backdrop` IS a bitwise NOT, and XOR against white is a
    // bitwise NOT -- and glLogicOp is core GL 1.1, exported straight from
    // opengl32.dll. Same pixels, no loader.
    //
    // THREE quads, not two, and they do not overlap: XOR is its own inverse,
    // so a full-height vertical bar crossing a full-width horizontal one would
    // invert the shared 2 x 2 centre twice and punch a hole clean through the
    // middle of the crosshair. The vertical is split into a stub above the
    // horizontal bar and one below it.
    // -----------------------------------------------------------------------
    void drawCrosshair() {
        if (menu_.open) return;  // the settings panel owns the middle of the screen

        // v1's 16 px, doubled (user). Scaled by WHOLE numbers past 1080p, so
        // the bars stay hard-edged at 4K rather than landing half on a pixel
        // and going grey.
        const int s = maxi(1, fbH_ / 1080);
        const int cx = fbW_ / 2, cy = fbH_ / 2;
        const int arm = 16 * s, half = 2 * s;  // 32 px across, 4 px thick
        ...
        glBegin(GL_QUADS);
        quadPx(cx - arm, cy - half, 2 * arm, 2 * half);      // horizontal, full width
        quadPx(cx - half, cy - arm, 2 * half, arm - half);   // vertical, above the bar
        quadPx(cx - half, cy + half, 2 * half, arm - half);  // vertical, below it
        glEnd();
        glDisable(GL_COLOR_LOGIC_OP);
        glEnable(GL_TEXTURE_2D);
    }

    // One axis-aligned rectangle in framebuffer pixels, emitted into an open
    // glBegin(GL_QUADS). Same pixel -> NDC mapping as blitPanel, y flipped.
    void quadPx(int x, int y, int w, int h) const {
        const float nx0 = float(x) / float(fbW_) * 2.0f - 1.0f;
        const float nx1 = float(x + w) / float(fbW_) * 2.0f - 1.0f;
        const float ny0 = 1.0f - float(y) / float(fbH_) * 2.0f;
        const float ny1 = 1.0f - float(y + h) / float(fbH_) * 2.0f;
        glVertex2f(nx0, ny0);
        glVertex2f(nx1, ny0);
        glVertex2f(nx1, ny1);
        glVertex2f(nx0, ny1);
    ...
    void drawMenu() {
        ...
        int ox = 0, oy = 0;
        menuOrigin(&ox, &oy);
        blitPanel(panel_, menuTex_, ox, oy, true);
    }
    ...
    std::string bakeDefaults() {
        ...
        std::fprintf(f,
            ...
            // kEye was missing here, and main.cpp reads defaults::kEye -- so the
            // FIRST bake anyone did rewrote this file without it and broke the
            // build. A bake has to emit every constant the file is expected to
            // declare, because it does not edit the file, it replaces it.
            ...
            "}  // namespace defaults\n"
            "}  // namespace v2\n",
            opt_.scale, base_.maxDepth, opt_.movingDepth, base_.exposure, opt_.speed,
            player_.eye, fov_,
