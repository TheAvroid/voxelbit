    ...
    void gather(const VoxelTerrain &t, TerrainMemo &memo, int bxIn, int bzIn) {
        ...
        for (int lz = -1; lz <= BRICK_VOX; ++lz)
            for (int lx = -1; lx <= BRICK_VOX; ++lx) {
                ...
                // THE OVERLOAD THAT SEES PETALS -- it wants the `top` and the
                // memo this loop already has, so a fallen petal costs nothing
                // more than the blade it stands in for.
                bm[padIdx(lx, lz)] =
                    t.bladeMaterial(i0 + lx, j0 + lz, sr[padIdx(lx, lz)], tm, memo);
            }
