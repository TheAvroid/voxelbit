...
//   5. IT SHRINKS AS THE DATA IMPROVES. A 1 m posting must invent about 18% of
//      what a 10.29 m one does -- (1/10.29)^0.75 -- because relief below the
...
int main(int argc, char **argv) {
    ...
    // ---- 5. a finer posting must invent less ---------------------------
    {
        ...
        const float want = std::pow(1.0f / 10.29f, 0.75f);
        printf("\n  posting factor   10.29 m -> %.4f    1 m -> %.4f  (want %.4f)\n",
               f10, f1, want);
        if (std::fabs(f10 - 1.0f) > 1e-4f || std::fabs(f1 - want) > 1e-4f) {
            printf("FAIL  posting scaling is wrong\n");
            ++bad;
        } else {
            printf("ok    1 m source invents %.0f%% of what 10.29 m does\n", 100.0 * f1);
        }
    }
