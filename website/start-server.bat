...
REM Double-click this file: it starts the no-cache server and opens voxelbit.html in your browser.
cd /d "%~dp0"

echo.
echo  Serving %CD%
echo  Open:  http://localhost:8080/voxelbit.html
echo  (Press Ctrl+C or close this window to stop.)
echo.

REM Open the engine once the server is up (detached 1 s delay so the first load doesn't race the server).
start "" cmd /c "timeout /t 1 >nul & start "" http://localhost:8080/voxelbit.html"
