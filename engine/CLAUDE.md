# v1 -- working here alongside another Claude tab
...
```powershell
$S = '<your-uuid>'
$T = 'C:\voxelbit\engine\tools\claude-session.ps1'
...
**1. NEVER kill a v1.exe that is not yours.** `status` prints `YOURS` or
...
engine is still alive. That survivor holds `Falcor.dll` and `v1.exe`, and the
...
running engine with `tasklist | find /I "v1.exe"`, and Git Bash puts its own
...
| symptom | cause |
|---|---|
| `LNK1104: cannot open file 'Falcor.dll'` / `'v1.exe'` | someone's engine is running |
...
Exit code is not enough — check `v1.exe`'s mtime moved:

```powershell
Get-Item C:\voxelbit\engine\build\bin\Release\v1.exe | Select-Object LastWriteTime
```
