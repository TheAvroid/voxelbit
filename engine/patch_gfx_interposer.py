...
sl.interposer.lib in place of d3d12.lib -- which v2 cannot do, because v2 does
...
REVERSIBILITY
-------------
The original is saved beside it as gfx.dll.v2-backup before anything is written,
and --restore puts it back. Nothing else in the tree is touched, so restoring
returns v2 to a stock Falcor exactly.
...
# v2 PATCHES THE STAGING COPY, NOT THE BUILD OUTPUT.
#
# v2 ships its own slang 2026.13 for cooperative vectors, staged at
# external\slang. deploycommon.bat re-copies %SlangDir%\bin\*.dll over the
# build every time Falcor relinks -- so a gfx.dll patched in the build
# directory works until the next build and then silently reverts, taking
# frame generation with it and leaving no diff to look at. Patched in the
# staging directory, every deploy carries it, and the shims sit beside it
# so the same robocopy takes them along.
BUILD = r'C:\voxelbit\engine\external\slang\bin'
GFX = os.path.join(BUILD, 'gfx.dll')
BACKUP = GFX + '.v2-backup'
...
FORWARDER = r'C:\voxelbit\engine\build\slgi\slgi.dll'
...
def restore():
    ...
    print('  v2 is back to stock Falcor (no frame generation)')
    return 0
...
if __name__ == '__main__':
    print('v2: slang-gfx / Streamline interposer patch')
