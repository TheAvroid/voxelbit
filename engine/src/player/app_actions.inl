...
// -----------------------------------------------------------------------------
    ...
    void drainVitals() {
        ...
        if (vitals_.deathWhy && deathAtMs_ < 0.0) {
            ...
            std::fflush(stdout);
            v2::crumbAction("the player died -- %s", vitals_.deathWhy);
            deathWhy_ = vitals_.deathWhy;
            deathAtMs_ = simMs_;
            vitals_.deathWhy = nullptr;
            // -- THE BODY LETS GO AND SO DOES THE PACK ------------------
            //
            // (user 2026-09-21: "have the player start floating upwards ...
            //  have all of the items in his inventory drop on the ground,
            //  levitating".)
            //
            // THE RISE is Player::dead -- no gravity, no ground, no jump --
            // and the input gate above it leaves only WASD, so the last few
            // seconds are a body drifting up with the camera still yours.
            //
            // THE DROPS ARE ALREADY LEVITATING and that is not new work: a
            // spilled item's arc "ends at the HOVER LINE, not at the ground"
            // -- see Drops::step, which is where fruit and wheat already come
            // to rest. Every carried tool is spilled on a bearing of its own
            // so the pack opens in a ring rather than stacking into one pile.
            player_.dead = true;
            int spilled = 0;
            for (int i = 0; i < held_.count(); ++i) {
                const Tool &t = held_.tool(i);
                if (!t.carried || t.models.empty()) continue;
                const float bearing = 6.2831853f * float(spilled) / 8.0f;
                drops_.spill(i, t.models[0], t.sx, t.sy, t.sz,
                             Vec3(player_.pos.x, player_.pos.y + 0.9f, player_.pos.z), bearing);
                ++spilled;
            }
            if (spilled)
                std::printf("v2: dropped %d item(s) where you fell\n", spilled);
        }
        if (deathAtMs_ >= 0.0 && simMs_ - deathAtMs_ >= kGameOverHoldMs) {
            deathAtMs_ = -1.0;
            deathWhy_.clear();
            // ...AND THE BODY IS SOLID AGAIN. Cleared before placeOnGround, or
            // the place lands a body that still refuses to be on the ground.
            player_.dead = false;
    ...
    void warmLife(const Vec3 &at, int seconds) {
        // THE SAME FLOOR THE FRAME LOOP HANDS THEM -- a warm-up that settles
        // the life onto a different surface from the one it will walk is a
        // warm-up that has to be undone.
        const WalkWorld gw = groundWorld();
        const auto groundAt = [&gw](float x, float z) { return walkGroundM(gw, x, z); };
    ...
    // -----------------------------------------------------------------------
    // -- ...AND NOT EVERY REBUILD IS A RESPAWN -------------------------
    //
    // (user 2026-09-20: "turning on snow teleports the player. prevent
    //  this.")
    //
    // TWO THINGS WERE FUSED IN HERE and only one of them belongs to a
    // setting. [G] means "give me a new world AND put me somewhere in it" --
    // that is the whole of the 2026-09-17 ask the block below records. The
    // snow toggle needs the first half and nothing else: it changes what the
    // ground is MADE of, so every resident chunk has to be re-meshed, but the
    // player asked to see snow where they are standing, not to be moved to a
    // glade somewhere else.
    //
    // `respawn` is the split. With it false the world is rebuilt around the
    // position you already hold and placeOnGround re-seats you on the new
    // surface -- which is not a no-op and must not be skipped: snow RAISES
    // the column (see VoxelTerrain::snowDepthAt), so a player left at their
    // old y would be standing inside it.
    void refreshWorld(bool respawn = true) {
            ...
            // ground under you.
            // WHERE THE REBUILD IS CENTRED. Respawning picks a new place and
            // primes around it; staying put primes around the place you are,
            // which is the same two lines with the choosing left out.
            if (respawn && !opt_.camGiven) chooseSpawn();
            const float atX = respawn ? opt_.camX : pos_.x;
            const float atZ = respawn ? opt_.camZ : pos_.z;
            world_.primeBlocking(Vec3(atX, pos_.y, atZ));
            player_.placeOnGround(walkWorld(), atX, atZ);
            pos_ = player_.eyePosition();
            nudgeOutOfSolids();
            ...
            std::printf("v2: world reloaded in %.0f ms -- edits dropped, life re-scattered, "
                        ...
                        std::chrono::duration<double, std::milli>(
                            ...
                            .count(),
                        respawn ? "respawned" : "stayed put", double(pos_.x), double(pos_.z));
            std::fflush(stdout);
    }
    ...
    bool breakWheat() {
        ...
        // -- ...AND IT COMES APART RATHER THAN VANISHING -------------------
        //
        // (user 2026-09-20: "when hitting the wheat with the hoe, have it break
        //  into peices instead of just making it dissapear".)
        //
        // THE STALKS ARE A BODY, the same way a tool's chip is: mow hands back
        // what it cut in spawnDebris' own layout, and this is the same call the
        // swing makes with it. SOFT, because wheat is -- it lands, settles and
        // can be walked up to, and kDebrisSoft is what the engine already means
        // by a fleshy thing rather than a rock.
        //
        // AFTER the drops below? No -- BEFORE, because mow is what decides
        // whether anything was cut at all, and the drops are paid only if it
        // was. The body is spawned here, from the same answer.
        //
        // -- kAbsorbNever, AND THAT IS WHY IT LOOKED LIKE NOTHING HAPPENED --
        //
        // (user 2026-09-20: "the wheat doesnt break into peices. fix." -- the
        // first version of this DID spawn the body and it was invisible.)
        //
        // It was given kArrowAbsorbM, 1.6 m, which is the reach a shaft states
        // so a chip knocked off something across a clearing does not sail back
        // to you. But you are STANDING OVER the wheat you just cut -- the swing
        // is two metres -- so the stalks were inside that radius on the frame
        // they were born and flew straight into the player. Spawned, absorbed,
        // gone, which on screen is exactly the vanishing this was meant to
        // replace.
        //
        // The stalks are SCENERY: the plant already pays a wheat and a seed as
        // drops, so a body that could also be collected would pay twice for one
        // swing. They lie there and rot on their own clock instead.
        std::vector<uint8_t> cut;
        int cutN = 0;
        Vec3 cutAt{0.0f, 0.0f, 0.0f};
        // ONLY THE CROP -- see World::mow's `only`. The lawn inside the
        // tuft's radius is not what a harvest takes, and taking it is what
        ...
        if (cutN > 0) {
            ...
            //
            // (user 2026-09-20: "the existing wheat strands should break
            //  apart like the fellen tree chunks".)
            //
            // ONE BODY, THEN World::shatterFelled ON IT -- which is the very
            // machinery a tree uses, not an imitation of it. What was here
            // was a hand-rolled split into four quadrant sub-cubes, and the
            // trouble with a second implementation of "break this into
            // chunks" is not that it is longer: it is that it does none of
            // what the first one learned. shatterFelled floods along the
            // voxels so a piece is CONNECTED, leaves the parent standing in
            // until the whole batch is built so nothing pops in a chunk at a
            // time, and hands the pieces the parent's velocity. Quartering a
            // cube by index has none of that.
            //
            // A REFUSAL IS NOT A FAILURE. shatterFelled breaks one body at a
            // time, so a swing taken while a tree is coming apart leaves the
            // sheaf whole -- it still falls, it is simply one piece. That is
            // the same contract the tree itself lives under.
            //
            // kAbsorbNever, AND THAT IS WHY IT ONCE LOOKED LIKE NOTHING
            // HAPPENED. The first version gave this kArrowAbsorbM -- 1.6 m --
            // and you are STANDING OVER the wheat you just cut, so the stalks
            // were inside that radius on the frame they were born and flew
            // straight into the player. The plant already pays a wheat and a
            // seed as drops; a body that could also be collected would pay
            // twice for one swing. These are scenery and rot on their own
            // clock.
            // -- ...AND NOW THEY ARE COLLECTED, ON PURPOSE -----------------
            //
            // (user 2026-09-21: "then make the player absorb it instantly".)
            //
            // THIS REVERSES THE DECISION THE NOTE ABOVE ARGUES FOR, and the
            // note stays because its reasoning is still the reasoning -- it was
            // right that the pieces flew straight in, it was wrong that that is
            // a failure. It is what was wanted.
            //
            // IT DOES NOT PAY TWICE. The absorb path flies the body to the eye
            // and retires it; the wheat and the seed are paid by the MOW, not
            // by collecting the stalk, so a collected chunk credits nothing and
            // the "pay twice for one swing" the note worries about cannot
            // happen. Checked rather than assumed -- see the absorb block in
            // updateDebris, which ends at retireDebris and nowhere else.
            // -- ...AND NOW IT SIMPLY GOES (user 2026-09-21: "just remove the
            //    wheat breaking apart mechanics. keep the code in tact, but
            //    just make the wheat dissapear like it did before and drop
            //    wheat item and a seed item") ---------------------------
            //
            // THE PLANT IS CUT AND PAID FOR AND THAT IS ALL. The mow has
            // already taken the stalks out of the world and thrown the two
            // drops; everything below this point existed to turn the cut
            // voxels into a BODY and then into chunks, and none of it is
            // wanted any more.
            //
            // NOTHING IS DELETED, WHICH IS THE ASK. `kWheatBreakBody` is the
            // one switch: turn it back on and the body is spawned, marked for
            // instant absorb and handed to shatterFelled exactly as before,
            // and every constant those steps read -- kWheatChunkVox,
            // Debris::absorbNow, markAbsorbNow -- is still here and still
            // carries its own notes. --wheat-test still exercises the whole
            // path when it is on.
            //
            // AND THE RULE IT LOOKS LIKE IT BREAKS, IT DOES NOT. "Whenever
            // something breaks apart from the static terrain it becomes a
            // rigid body" is about severed GEOMETRY that would otherwise hang
            // in the air or vanish mid-world. A mown stalk is neither: it is
            // consumed by the swing that paid for it, the way a picked fruit
            // is, and the voxels are gone from the grid before this line.
            static constexpr bool kWheatBreakBody = false;
            const int body = kWheatBreakBody
                                 ? world_.spawnDebris(physics_, cut, cutN, cutAt, kNoVel, kNoVel,
                                                      simMs_, 0, nullptr, uint8_t(kDebrisSoft),
                                                      kAbsorbAnywhere)
                                 : -1;
            int made = 0;
            if (body >= 0) {
                // SCENERY, AND SAID BEFORE THE SHATTER because the pieces
                // inherit it -- see PendingChunk::scenery. Without this they
                // are born as a felled tree's chunks are, loot with a 2.6 m
                // reach, and you are standing on top of them.
                // NOT markScenery ANY MORE -- scenery is exactly what the
                // absorb gate refuses, so marking it here is what made the
                // stalks lie there. See Debris::absorbNow for the other half:
                // the half-second wait every other body serves is waived, so
                // the piece leaves on the frame it appears.
                world_.markAbsorbNow(body);
                // The word the shatter's own report prints. It is a tree's
                // field and this is not a tree, so it is set rather than left
                // saying whatever felled the last oak.
                world_.setFellWhy("the hoe");
                // EIGHT VOXELS A PIECE, NOT THE TREE'S 175 -- see the
                // chunkVox note on shatterFelled. A stalk is about twenty
                // voxels tall and unconnected to its neighbours, so at the
                // tree's size every strand came away whole: the tuft broke
                // up and the strands did not. Eight puts two or three pieces
                // in each one, which is a cut plant rather than a bundle of
                // sticks.
                made = world_.shatterFelled(physics_, body, simMs_, kWheatChunkVox);
            }

            // WHETHER IT ACTUALLY BECAME ONE, and WHERE. spawnDebris returns
            // -1 when it refuses, and a refusal here is invisible on screen --
            // the stalks simply carry on vanishing, which is the report this
            // answers. The cube's side says why: an empty one is mow not
            // filling it, a full one with -1 is the body pool or PhysX.
            int lit = 0;
            for (uint8_t v : cut) lit += (v != mat::AIR) ? 1 : 0;
            std::printf("v2: wheat cut -- %d stalk voxels in a %d cube, body %d, "
                        "%d piece(s)\n",
                        lit, cutN, body, made);
            std::fflush(stdout);
        }
