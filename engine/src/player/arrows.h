...
class Arrows {
  public:
    struct Shaft {
        ...
        // -- WHAT IT IS STANDING IN, WHEN THAT THING MOVES ----------------
        //
        // (user 2026-09-20: "have the arrow go with whatever it landed on.
        //  this was done in v1 ... it should fall with the chunk for example,
        //  or go with the lifes peices. currently its stays in the air when
        //  shooting a fly".)
        //
        // A shaft used to be pinned to the world point where it struck, so
        // whatever it hit fell, shattered or was carried out from under it and
        // left it hanging in clear air. `ride` is the loose body it is buried
        // in; `rideGen` is that body's birth stamp, which is what keeps a
        // RECYCLED slot from being mistaken for the same piece still carrying
        // the arrow -- v1 uses its own born stamp the same way and for the
        // same reason.
        //
        // THE OFFSET IS IN THE BODY'S OWN FRAME, taken once when it struck and
        // replayed out of it every frame after, so the shaft keeps its spot on
        // the piece and TURNS with it. A yaw-only carry would do for an animal
        // and not for a chunk: a felled trunk topples about a HORIZONTAL axis,
        // and a shaft that stayed level while the wood it is buried in lay
        // down would read as a bug. v1 makes exactly that point.
        int ride = -1;
        double rideGen = 0.0;
        Vec3 rideOff{0, 0, 0};   // where on the piece, in the piece's frame
        Vec3 rideDir{0, 0, 1};   // ...and which way it pointed, in that frame
        // ONE FRAME OF GRACE. A body that SHATTERS is spliced out and its
        // children built in the same tick, but not necessarily before this is
        // next asked -- so reaping on the first miss would delete a shaft that
        // is about to be re-attached to the piece it is really in. Two
        // consecutive misses means nothing claimed it. v1's rule and its
    ...
    void update(float dt, const WalkWorld &w, const LifeF &hitLife = nullptr) {
        ...
        for (Shaft &a : shafts_) {
            ...
            // -- ...AND WATER IS A SURFACE, NOT A HOLE IN THE WORLD ---------
            //
            // (user 2026-09-20: "when shooting an arrow in the water, it
            //  glitches, it darts in a straight line".)
            //
            // NOTHING IN THIS FILE HAD EVER HEARD OF WATER. insideWorld asks
            // TerrainProbe::topVox, which is the LAKE BED -- water is not a
            // solid to it -- so a shaft loosed at a lake passed through the
            // surface at full speed, crossed the whole body of it in a straight
            // line and buried itself in the mud. Through clear water you watch
            // the entire flight, which is the dart.
            //
            // v1 rests anything that lands on water AT THE WATERLINE (`y:
            // onWater ? WL : hmap[...]`), so that is what this does. One memo
            // for the whole flight rather than one per substep: a shaft crosses
            // a handful of columns and the memo is the terrain's own per-column
            // cache.
            TerrainMemo wmemo;
    ...
    // HOW MANY SHAFTS ARE BEING CARRIED, and where the first of them is.
    // The one number that says whether a shaft went with what it hit or stayed
    // in the air where the thing used to be -- see --kill-test.
    int riding(Vec3 *at = nullptr) const {
        int n = 0;
        for (const Shaft &a : shafts_) {
            if (a.ride < 0 || !a.stuck) continue;
            if (n == 0 && at) *at = a.pos;
            ++n;
        }
        return n;
    ...
    // ...and WHICH shaft each of those was, so the caller can hand it the
    // thing it landed in. Same order, same length.
    const std::vector<int> &landedShafts() const { return landedIdx_; }

    // =======================================================================
    // A SHAFT GOES WITH WHAT IT STRUCK.
    // =======================================================================
    //
    // Two calls, because they happen at different moments and knowing which is
    // which is most of the bug: ATTACH once, on the tick it landed and after
    // whatever it hit has broken into pieces; REPLAY every frame after.
    //
    // Neither of them knows what a rigid body IS -- the caller answers that,
    // because Arrows has a WalkWorld and nothing else. See World::bodyNear.
    // -----------------------------------------------------------------------

    // `at` is a lookup: given a world point, which body and which generation.
    // Returns the number of shafts that found one.
    template <typename AtF>
    int attachRides(const AtF &at) {
        int n = 0;
        for (int i : landedIdx_) {
            if (i < 0 || i >= int(shafts_.size())) continue;
            Shaft &a = shafts_[size_t(i)];
            if (!a.stuck || a.ride >= 0) continue;
            double gen = 0.0;
            Vec3 pos{0, 0, 0};
            float q[4] = {0, 0, 0, 1};
            const int b = at(a.pos, &gen, &pos, q);
            if (b < 0) continue;
            a.ride = b;
            a.rideGen = gen;
            a.rideMiss = 0;
            // INTO THE BODY'S FRAME ONCE, so the replay is a single rotation
            // back out of it -- the shape v1's creature sweep uses.
            a.rideOff = qRot(qConj(q), a.pos - pos);
            a.rideDir = qRot(qConj(q), a.dir);
            ++n;
        }
        return n;
    }

    // `pose` answers "where is body i now", false if it has gone.
    template <typename PoseF>
    void rideStep(const PoseF &pose) {
        for (Shaft &a : shafts_) {
            if (a.ride < 0 || !a.stuck) continue;
            Vec3 pos{0, 0, 0};
            float q[4] = {0, 0, 0, 1};
            if (!pose(a.ride, a.rideGen, &pos, q)) {
                // ...AND IT GOES THE WAY AN ARROW IN A DEAD ANIMAL GOES. Two
                // misses, not one -- see Shaft::rideMiss.
                if (++a.rideMiss < 2) continue;
                a = Shaft{};
                continue;
            }
            a.rideMiss = 0;
            a.pos = pos + qRot(q, a.rideOff);
            a.dir = qRot(q, a.rideDir);
            // THE REST TIMER IS THE PIECE'S NOW. A shaft standing in the
            // ground gives its slot up after kArrowRestSec; one riding a body
            // is retired by the body going instead, which is the whole point
            // of the ride -- otherwise a chunk lands and the arrow in it
            // vanishes while you watch.
            a.age = 0.0f;
        }
    }

    // -- the quaternion, which is all the maths this file needs --------------
    static Vec3 qRot(const float q[4], const Vec3 &v) {
        const float x = q[0], y = q[1], z = q[2], w = q[3];
        const Vec3 u(x, y, z);
        const float uv = u.x * v.x + u.y * v.y + u.z * v.z;
        const float uu = x * x + y * y + z * z;
        const Vec3 cr(u.y * v.z - u.z * v.y, u.z * v.x - u.x * v.z, u.x * v.y - u.y * v.x);
        return Vec3(2.0f * uv * u.x + (w * w - uu) * v.x + 2.0f * w * cr.x,
                    2.0f * uv * u.y + (w * w - uu) * v.y + 2.0f * w * cr.y,
                    2.0f * uv * u.z + (w * w - uu) * v.z + 2.0f * w * cr.z);
    }
    struct Quat {
        float q[4];
        operator const float *() const { return q; }
    };
    static Quat qConj(const float q[4]) { return Quat{{-q[0], -q[1], -q[2], q[3]}}; }
