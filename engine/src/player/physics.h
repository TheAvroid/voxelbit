...
class Physics {
  public:
    ...
    // -- HELD EXACTLY WHERE IT IS, AND NOT FALLING ----------------------
    //
    // (user 2026-09-20: "when the tree breaks into chunks, work on making it a
    //  smooth and clean transition from the falling tree to chunks".)
    //
    // A shatter builds its pieces over several frames and keeps them UNSEEN
    // behind the standing tree (see World::revealShatter). They were still
    // being SIMULATED the whole time, so by the reveal the early ones had
    // fallen a few centimetres and turned slightly, and the frame where the
    // tree and its chunks overlap showed the two a little out of register --
    // which is the shudder at the changeover.
    //
    // Kinematic is the exact state wanted: the body keeps its pose, ignores
    // gravity and contacts, and costs the solver nothing. Everything is
    // released together on the reveal frame, which is also what stops the
    // whole set resolving its own interpenetration in one impulse.
    // -----------------------------------------------------------------------
    // LET A SETTLED PIECE GO TO SLEEP -- and not one moment before.
    //
    // (user 2026-09-21: "when cutting down a tree my screen started lagging".)
    //
    // addChunkBody sets setSleepThreshold(0) so a piece NEVER sleeps, and the
    // comment there is right about why: "a chip is collected half a second
    // after it comes loose, and a body that falls asleep in that time sits
    // frozen for the rest of it". That reasoning is about ONE chip in the first
    // half-second of its life.
    //
    // IT WAS BEING APPLIED TO FOUR HUNDRED AND FORTY-EIGHT OF THEM FOR A FULL
    // MINUTE. A felled tree may now take kFellSlotsShare = 448 slots, so a
    // single cut hands PhysX that many permanently-awake CCD bodies with 8/4
    // solver iterations, and they stay awake lying still on the ground until
    // kFelledLifeMs retires them. That is the lag, and none of it is motion
    // anybody can see.
    //
    // So the threshold is raised LATER instead of at birth: World::updateDebris
    // calls this once a piece is past the age at which it was going to be
    // picked up mid-wobble. A sleeping body still wakes on contact, so a piece
    // you walk into or land on behaves exactly as before.
    void allowSleep(int h, float threshold) {
#if V2_HAS_PHYSX
        if (h < 0 || size_t(h) >= bodies_.size() || !bodies_[size_t(h)]) return;
        bodies_[size_t(h)]->setSleepThreshold(threshold);
#else
        (void)h;
        (void)threshold;
#endif
    ...
    // ...AND THE SAME THING ADDED RATHER THAN SET. nudgeSpin below REPLACES
    // the angular velocity, which is right for a one-shot hinge and wrong for
    // anything applied every frame: setting it each tick pins the rate instead
    // of building it, and the body can never go over faster than the constant.
    // See kFellLeanAcc.
    void addSpin(int h, const Vec3 &axis, float radPerSec) {
#if V2_HAS_PHYSX
        if (h < 0 || size_t(h) >= bodies_.size() || !bodies_[size_t(h)]) return;
        physx::PxRigidDynamic *a = bodies_[size_t(h)];
        const physx::PxVec3 w = a->getAngularVelocity();
        a->setAngularVelocity(physx::PxVec3(w.x + axis.x * radPerSec, w.y + axis.y * radPerSec,
                                            w.z + axis.z * radPerSec));
        a->wakeUp();
#else
