...
class Player {
  public:
    ...
    // The EYE is under the surface -- see the note where it is set. Not the
    // same question as swimming().
    bool submerged() const { return submerged_; }
    ...
    // -- THE BODY LETS GO AND GOES UP -----------------------------------
    //
    // (user 2026-09-21: "when the player dies, have the player start floating
    //  upwards ... the player can still control the player with the wasd keys.
    //  only the wasds should work breifly before being respawned".)
    //
    // Set by the app on the frame the bar empties and cleared at the respawn.
    // While it is on the body does not fall, does not land and cannot be
    // jumped or crouched: the vertical is this rise and nothing else, and the
    // horizontal is whatever WASD asks for. It is deliberately NOT `fly`, which
    // is a debug mode with its own speeds, its own hazard exemptions and a
    // descent on the same key.
    bool dead = false;
    float deadRiseV = 1.6f;   // m/s -- a drift, not a launch
    // -----------------------------------------------------------------------
    ...
    void update(const WalkWorld &w, Vec3 move, bool sprint, bool jump, bool down, bool crouch,
        ...
        if (fly) {
            ...
            // -- ...AND WHETHER THE HEAD IS UNDER -----------------------
            //
            // (user 2026-09-21: "allow the player to drown and die
            //  underwater".)
            //
            // SWIMMING IS NOT DROWNING. `swimming_` asks about a point partway
            // up the body -- it is what decides the stroke and the step-up --
            // and a player standing chest-deep in a lake is swimming and
            // breathing. Drowning is a question about the EYE, which is the
            // one point that has to be above the surface, and it is asked with
            // the same lake sample so the two cannot disagree about a column.
            submerged_ = inLake(lake, pos.y + eyeHeight());
            ...
            // DEAD COMES FIRST, ahead of the swim arm: drowning is one of the
            // ways to die, and a corpse that kept treading water would never
            // leave the lake it drowned in.
            if (dead) {
                // THE ONE VERTICAL A DEAD BODY HAS. No gravity, no ground test,
                // no landing -- it rises through whatever is above it, which is
                // the point: dying in a pit should not leave you lying in it.
                vy = deadRiseV;
                pos.y += vy * dt;
                onGround = false;
            } else if (swimming_) {
