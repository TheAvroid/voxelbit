...
// -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // -----------------------------------------------------------------------
    // WHERE A TAKE GOES, AND WHY IT IS NOT JUST "recordings".
    //
    // Relative to the working directory is right for a developer -- that is
    // the repo, and every take this engine has ever made is in
    // voxelbit/recordings. It is WRONG for a player: the launcher runs the
    // game from its unpack root under %LOCALAPPDATA%, so a packaged take was
    // written to a folder named after a build hash that the next build is
    // entitled to delete. The recording worked; it was simply gone.
    //
    // VOXELBIT_HOME is the directory voxelbit.exe sits in -- the one the
    // player chose -- and the launcher sets it. Unset for a direct run, which
    // keeps the developer's behaviour exactly as it was.
    //
    // IT FALLS BACK RATHER THAN FAILING. voxelbit.exe may sit somewhere
    // unwritable (Program Files, a read-only share), and a take that cannot be
    // filed is still a take worth keeping -- so the working directory remains
    // the last resort.
    // -----------------------------------------------------------------------
    static std::string recordingsDir() {
        std::error_code ec;
        if (const char *home = std::getenv("VOXELBIT_HOME")) {
            if (*home) {
                const std::string want = std::string(home) + "/recordings";
    ...
    static std::string outputPath(const char *fmt, int *counter) {
        std::error_code ec;
        const std::string into = recordingsDir();
        const bool dir = !into.empty();
        char name[64];
        for (int guard = 0; guard < 10000; ++guard) {
            std::snprintf(name, sizeof(name), fmt, *counter);
            ++*counter;
            std::string path = dir ? (into + "/" + name) : std::string(name);
            if (!std::filesystem::exists(path, ec)) return path;
        }
        return dir ? (into + "/" + name) : std::string(name);
    }
    ...
    void toggleRecording() {
        ...
        std::printf("v2: recording to %s -- %dx%d @ %d fps (F9 again to stop)\n", name,
