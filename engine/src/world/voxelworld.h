...
// That is the same failure the broadleaf floor value was introduced to fix one level
...
constexpr uint8_t BGRASS_FLOOR_COUNT = 6;  // 32..37 -- the whole ramp now
...
// -- 10, AND 18 WAS STILL TOO WIDE ------------------------------------------
//
// (user 2026-09-20: "fix the color palete on the gecko" / "... on the scorpion
//  too".)
//
// 18 caught the cobra's tan landing on an olive (25.4 deg out) and let both of
// these through:
//
//   scorpion  (218, 95, 61) rust  -> (224, 78, 78) crimson   13.0 deg
//   gecko     (255,240,161) yellow-> (255,221,180) peach     17.5 deg
//
// and both are the SAME failure the cobra was: the channel relationship flips.
// A rust has b well under g; the crimson it landed on has g == b exactly. Ten
// degrees refuses both with room to spare and still keeps every match that was
// already right (the cobra's tans are 0.5 deg from theirs).
inline constexpr float kModelHueDeg = 10.0f;
...
class Palette {
  public:
    ...
    uint8_t nearestModelColor(const std::array<uint8_t, 4> &c, int tol,
        ...
        // -- AND IF NOTHING IN THE TABLE HAS THIS HUE, SAY SO -------------
        //
        // This used to fall back to `hitAny` -- the nearest by distance,
        // whatever its hue -- on the reasoning that returning 0 mints, and a
        // model that fails to mint renders as AIR.
        //
        // IT CANNOT. forModelColor mints only while there is room, and once
        // the table is full it SNAPS to the nearest rather than handing back
        // AIR ("the wrong colour is a bug you can see, and a wrapped id is one
        // you cannot"). So 0 here means "no entry has this hue" and the two
        // outcomes are: mint the colour as authored, or snap to the nearest --
        // which is exactly what the fallback did, only now it happens after
        // minting has been tried instead of instead of it.
        //
        // That is the whole of the scorpion fix. There is no rust in this
        // table, so the fallback shared a crimson for ever; refusing lets the
        // rust be minted, and there was room for it.
        (void)hitAny;
        (void)bestAny;
        return hit;
    }
    ...
    // -- ...AND `refused` IS HOW A CALLER FINDS OUT IT DID NOT GET ONE -----
    //
    // (user 2026-09-20: "What if I want each object to be on its own 256
    //  palette?")
    //
    // Set true, if the caller passes one, exactly when the table is full and
    // this call takes the snap-to-nearest path below. The RETURN VALUE cannot
    // say that: the snap deliberately hands back a real id, because "the wrong
    // colour is a bug you can see and a wrapped id is one you cannot", and the
    // counters it bumps are totals for the start-up report rather than an
    // answer about THIS colour.
    //
    // What it is for: a model with a table of its own can take the colour the
    // wood had no room for and put it in its OWN 255 instead of wearing the
    // nearest thing the wood happens to own. See World::privateModelEntry.
    uint8_t forModelColor(const std::array<uint8_t, 4> &c, bool conifer = true,
                          bool exact = false, int matchTol = 0,
                          bool avoidFoliage = false, bool *refused = nullptr) {
        ...
        if (next_ >= mat::COUNT) {
            // THE ONLY PLACE THIS IS SET. Everything below is the fallback,
            // and a caller that can do better than a snap wants to know it is
            // about to get one. See the note over the signature.
    ...
    // The narrower fact, exposed for the same reason isFoliage is: a private
    // entry has to be placed at an id whose CLASSIFICATION already matches, or
    // a reader holding a bare id gets the wrong answer. See
    // World::privateModelEntry.
    bool isBlossom(uint8_t id) const { return blossomId_[id] != 0u; }
    ...
    // never about it.
    // -- ...AND THEN THE ASK BECAME "MATCH THE TREES" -----------------
    //
    // (user 2026-09-20: "how about for the grass/strands, you have it match
    //  the color of the trees. with the exception of the cherry forest.")
    //
    // WHICH IS 1.0, AND THE NOTE ABOVE IS WHY THAT IS THE LITERAL ANSWER. A
    // foliage ENTRY is what the canopy is drawn with -- forModelColor has
    // already done its 1.7x shade lift by the time it lands in the table --
    // so "the colour of the trees" is that entry's albedo, unscaled. Every
    // number below 1 was an argument about LIGHTING (a needle sits in shade, a
    // blade stands in the sun) dressed up as an argument about colour, and the
    // ask is about colour.
    //
    // ONE CONSTANT NOW, NOT TWO. The pine floor and the broadleaf floor were
    // 0.52 and 0.72 because the two woods wanted different amounts of that
    // lighting correction; with the correction gone there is nothing left for
    // them to disagree about. The woods still differ -- they are built from
    // different foliage sets, which is the difference that was always real.
    //
    // THE CHERRY EXCEPTION IS ALREADY STRUCTURAL and needs no number: the
    // blossom entries are skipped where the foliage sets are gathered (see
    // blossomId_), so the cherry wood's floor is built from the same green
    // leaf litter the oak's is and matching harder cannot turn it pink.
    //
    // WHAT THIS GIVES UP is the standing warning under GRASS_0 about the
    // conifer floor glowing under a dark canopy. It is a real risk and the
    // old value was defensible; it is also exactly what was asked for, and the
    // one number to move to undo it is here.
    static constexpr float kFloorMatchV = 1.0f;
    // -- HOW FAR APART THE SHADES OF A BLADE STAND --------------------
    //
    // THIS HAS BEEN ASKED FOR THREE TIMES IN THREE DAYS, twice in opposite
    // directions, and the whole history is worth one paragraph because it is
    // what the design below is FOR:
    //
    //   2026-09-17  "in the oak forest, I want more contrast between the
    //               terrain/grass voxels ... it looks like one color"
    //   2026-09-20  "lower the contrast of the grass strands across biomes.
    //               keep them to the lighter side"
    //   2026-09-20  "now all the grass is the same color this is wrong"
    //
    // TWO OF THOSE THREE ARE THE SAME REPORT, and the middle one was never a
    // request for LESS separation -- it was a request for the separation to
    // sit higher up. Answering it with an exponent under one did both at once
    // and undid the first. That is why value and colour are two knobs now:
    //
    //   kGrassSpread     how many times brighter the top of the ramp is than
    //                    its bottom. VALUE only, anchored at the light end.
    //   kGrassDarkTrim   which part of the source the hues come from, so the
    //                    ramp is built out of lighter foliage rather than the
    //                    near-black. COLOUR only.
    //
    // BOTH WOODS, which is what "across biomes" asks for -- fillGrassRamp is
    // the one fill and the pine ramp goes through it too. The floor scatters
    // over the same entries and the straw ramps are built FROM them
    // (fillWheatRamp), so the terrain follows without a second edit.
    // -- ...AND THEN IT MEASURED AS ONE COLOUR AGAIN ------------------
    //
    // (user 2026-09-20: "now all the grass is the same color this is wrong.
    //  use a nice light green gradient on the grass strands and a nice grass
    //  gradient on the terrain as well.")
    //
    // 0.72 WAS CHOSEN BY REASONING AND IT WAS WRONG BY A FACTOR OF TWO.
    // Measured with V2_GRASS_RAMP, both ramps came out at a 1.29x spread
    // across the six shades a blade can reach -- FLATTER than the 1.53x that
    // was reported as "it looks like one color" in the first place. An
    // exponent has no meaning without the source's own spread beside it, and
    // nothing was measuring that.
    //
    // AND AN EXPONENT COULD NOT HAVE FIXED IT ANYWAY. Measured, shades 0 and
    // 1 of the broadleaf ramp were 0.3309 and 0.3315 -- a fifth of a percent
    // apart. That is not the contrast control failing, it is the SOURCE:
    // sampling six points across a sorted list of foliage albedos lands two
    // of them on the same colour wherever that list has a flat stretch, and
    // no power about any pivot separates two numbers that are already equal.
    // "every voxel going upwards is a different shade" cannot be built on a
    // transform of the source's distribution; it has to be built on a ramp
    // that states its own.
    //
    // SO THE SHADES ARE PLACED, NOT DERIVED. kGrassSpread is how many times
    // brighter the top of the ramp is than the bottom, and the shades between
    // are spaced GEOMETRICALLY across it -- equal ratios, which is what the
    // eye reads as equal steps. Every shade differs from its neighbour by
    // construction, whatever the art happens to contain.
    //
    // 2.1 IS THE SPREAD, not the brightness: it is anchored at the TOP, so
    // the lightest shade is exactly the light green the ramp already ends on
    // and the expansion is spent entirely below it. That is both halves of
    // what has been asked twice -- the light end is kept and the gradient
    // comes back -- and it is why this pivots on the last entry rather than
    // the middle. Over six shades it is a 15.7% step, which is several times
    // the smallest difference the eye resolves at these values.
    //
    // kGrassDarkTrim is UNCHANGED and does a different job: it decides which
    // part of the source the HUES are drawn from, so the ramp is built out of
    // the lighter foliage rather than the near-black. The spread then says
    // how far apart to place them. Trimming is about colour, spread is about
    // value, and conflating the two is what produced 0.72.
    static constexpr float kGrassSpread = 2.1f;
    static constexpr float kGrassDarkTrim = 0.38f;
    ...
    // `valueScale` is how much of the FOLIAGE's value the floor keeps; see
    // kFloorMatchV for why it is 1.0 for both woods now, and for what the
    // two numbers it replaced were actually correcting for.
    void fillGrassRamp(uint8_t base, uint8_t count, std::vector<uint8_t> src,
                       float valueScale, float spread = 1.0f) {
        ...
        for (int k = 0; k < int(count); ++k) {
            ...
            if (src.empty()) {
                ...
                // THE DARK END IS TRIMMED OFF -- see kGrassDarkTrim. `u` is
                // where this shade sits across the ramp; the source is then
                // sampled over [trim, 1] of its own range rather than all of
                // it, so the darkest foliage no longer reaches the floor.
                const float u =
                    (int(count) > 1) ? float(k) / float(int(count) - 1) : 0.0f;
                const float t = (kGrassDarkTrim + (1.0f - kGrassDarkTrim) * u) *
                                float(src.size() - 1);
        ...
        // -- ...AND THE SHADES ARE THEN PLACED, NOT SAMPLED --------------
        //
        // See kGrassSpread for why this replaces the power-about-a-pivot that
        // was here: an exponent transforms whatever spacing the source has,
        // and the source has two shades that are the same colour.
        //
        // GEOMETRIC, NOT LINEAR. Equal RATIOS of luminance are what the eye
        // reads as equal steps, so the tenth of the ramp nearest the ground
        // gets as much visible separation as the tenth at the tip. Linear
        // spacing would crowd every step into the dark end and the bottom
        // third of a blade would read as one colour -- which is the report,
        // arrived at a different way.
        //
        // ANCHORED AT THE TOP. The last entry keeps the luminance the source
        // gave it and everything below is placed under it, so the light green
        // the ramp ends on is not something this decides -- it is still the
        // wood's own foliage at the floor's value.
        //
        // ONE SCALAR PER SHADE, so the hue cannot move -- the identical
        // argument valueScale makes above, and the reason this is a value
        // control rather than a recolour.
        if (spread > 1.0f && count > 1) {
            const float hi = luminance(look_[base + int(count) - 1].albedo);
            if (hi > 1e-5f) {
                const float lo = hi / spread;
                for (int k = 0; k < int(count); ++k) {
                    MaterialLook &m = look_[base + k];
                    const float l = luminance(m.albedo);
                    if (l <= 1e-5f) continue;
                    const float u = float(k) / float(int(count) - 1);
                    const float want = lo * powf(spread, u);
                    m.albedo = m.albedo * (want / l);
                    // ...AND NOTHING MAY REFLECT MORE THAN IT RECEIVES.
                    // Anchoring at the top means nothing is lifted past where
                    // the source put it, so this cannot fire today -- it is
                    // kept because the anchor is a choice and the ceiling is
                    // not. Scaled by the WHOLE colour rather than clamped per
                    // channel, or the cap would pull green down toward red and
        ...
        // -- WHAT THE RAMP ACTUALLY CAME OUT AS -------------------------
        //
        // (user 2026-09-20: "now all the grass is the same color this is
        //  wrong" -- after a contrast of 0.72 was set from reasoning rather
        //  than from measurement.)
        //
        // A CONTRAST CONSTANT CANNOT BE CHOSEN WITHOUT THE NUMBERS. The
        // source is whatever foliage the models happen to carry, so the same
        // exponent gives a different spread in each wood and "about 0.7"
        // means nothing on its own. This prints the shades and the RATIO
        // across the ones a short blade can reach, which is the quantity the
        // report is actually about -- see the 1.53x that read as one colour.
        //
        // Env-gated rather than a flag: main.cpp's arg chain is at MSVC's
        // nesting limit (see the v2-water-panel note) and this is a bench
        // instrument, not a feature.
        if (std::getenv("V2_GRASS_RAMP")) {
            std::printf("[ramp] base %3d  count %2d  spread %.2f  trim %.2f\n",
                        int(base), int(count), double(spread), double(kGrassDarkTrim));
            float lo = 1e9f, hi = -1e9f;
            for (int k = 0; k < int(count); ++k) {
                const Vec3 a = look_[base + k].albedo;
                const float l = luminance(a);
                // THE FIRST SIX ARE THE MEASUREMENT. A short blade is 3 to 6
                // voxels and reads one shade per voxel, so those are the only
                // entries the floor and the grass on it ever wear.
                if (k < 6) {
                    lo = minf(lo, l);
                    hi = maxf(hi, l);
                }
                std::printf("        %2d  rgb %.3f %.3f %.3f   srgb %3d %3d %3d   lum %.4f\n",
                            k, double(a.x), double(a.y), double(a.z),
                            int(255.0f * powf(clampf(a.x, 0.0f, 1.0f), 1.0f / 2.2f) + 0.5f),
                            int(255.0f * powf(clampf(a.y, 0.0f, 1.0f), 1.0f / 2.2f) + 0.5f),
                            int(255.0f * powf(clampf(a.z, 0.0f, 1.0f), 1.0f / 2.2f) + 0.5f),
                            double(l));
            }
            std::printf("        visible 0..5: %.4f .. %.4f = %.2fx\n", double(lo),
                        double(hi), double(hi / maxf(lo, 1e-6f)));
            std::fflush(stdout);
        }
    }
    ...
    void deriveGroundFromTrees() {
        ...
        // PIVOTED AT THE LIGHTEST SHADE, which is what "keep them to the
        // lighter side" means as an instruction to this function. A power about
        // the MIDDLE compresses both ends toward it, so the top comes down with
        // the bottom and the ramp gets flatter AND darker at its brightest --
        // half the ask, backwards. About the last entry the lightest shade is
        // its own pivot and cannot move; everything below it is drawn up.
        fillGrassRamp(mat::GRASS_0, mat::GRASS_COUNT, foliage, kFloorMatchV, kGrassSpread);
        ...
        // the note over kFloorMatchV is a standing warning about brightening
        // that floor.
        fillGrassRamp(mat::BGRASS_0, mat::BGRASS_COUNT,
                      bfoliage.empty() ? foliage : bfoliage, kFloorMatchV, kGrassSpread);
        ...
        // -- WHAT THE TWO GRASS RAMPS ACTUALLY CAME OUT AS ----------------
        //
        // The one number that says whether "lower the contrast, keep the light
        // side" happened: the darkest and lightest shade of each ramp, and the
        // ratio between them. A render answers this badly -- the floor is lit,
        // shadowed and half covered in flowers -- and these are the albedos
        // themselves, before any of that.
        for (int w = 0; w < 2; ++w) {
            const uint8_t b0 = w ? mat::BGRASS_0 : mat::GRASS_0;
            const uint8_t cN = w ? mat::BGRASS_COUNT : mat::GRASS_COUNT;
            float lo = 1e9f, hi = -1e9f;
            for (int k = 0; k < int(cN); ++k) {
                const float l = luminance(look_[b0 + k].albedo);
                lo = minf(lo, l);
                hi = maxf(hi, l);
            }
            std::printf("  grass    %-9s %d shades, luminance %.3f .. %.3f  (spread %.2fx)\n",
                        w ? "broadleaf" : "conifer", int(cN), double(lo), double(hi),
                        double(lo > 1e-5f ? hi / lo : 0.0f));
        }
...
// -- ...AND HOW MANY OF THOSE THE CODE CAN ACTUALLY SAY -----------------
//
// (user 2026-09-21: "the wheat is sometimes discolored, theres a green voxel
//  where there shouldnt be".)
//
// THIRTY-ONE, NOT THIRTY-TWO, AND THE DIFFERENCE WAS A BUG. The strand field
// is five bits and ZERO MEANS "not a blade" -- the sentinel a flower stem
// rides on -- so there are 31 codes left to say WHICH ROW a blade starts on,
// not 32. strandCodeFor stored `1 + (baseRow & 31)`, which is 1..32, and 32
// does not fit in five bits: every ground row at 31 mod 32 packed to 0 and
// came back reading "not a blade".
//
// WHAT THAT LOOKED LIKE. The shader gates the whole height gradient on
// `strand != 0`, so those stalks skipped it and fell through to the scatter,
// which picks a shade anywhere in the ramp by hash. On grass that is green
// scattered over green and invisible. On WHEAT the ramp runs green at the
// root to straw at the tip, so a scattered shade is a green voxel partway up
// a straw stalk -- one column in thirty-two, which is exactly "sometimes".
//
// A PHASE, NOT A HEIGHT. This only has to exceed the tallest blade (20) for
// the wrap to be unambiguous, which 31 does with room to spare.
constexpr int STRAND_PHASES = 31;
static_assert(STRAND_PHASES <= TRI_STRAND_MASK,
              "the strand code must fit the field with 0 left for the sentinel");
static_assert(STRAND_PHASES > 20, "the wrap must clear the tallest blade");
// How many voxels one step of the stored row covers. See the note above.
// -- ONE SHADE PER VOXEL -------------------------------------------------
//
// (user 2026-09-20: "make sure each of the grass strands contains a smooth
//  gradient where every voxel going upwards is a different shade. with the
//  lightest shade on the top".)
//
// TWO ONCE, AND THAT IS WHAT WAS WRONG WITH IT: at a step of two a six-voxel
// blade wore three shades in pairs, so half the voxels going up matched the one
// below them. One shade per voxel is what the ask says and what a six-shade
// ramp over a three-to-six voxel blade is FOR.
//
// THE ENCODING STILL FITS. strandCodeFor stores the blade's base row divided by
// this step in TRI_STRAND_MASK's five bits, so the shader's row arithmetic is
// correct as long as the wrap -- STRAND_MAX_ROWS voxels -- is longer than the
// tallest blade. At a step of one that wrap is 32 voxels against a tall tuft's
// 20, so it still clears it.
//
// THE STRAW RAMPS KEEP THE PAIRING, in the shader rather than here: ten shades
// over a 20-voxel tuft only reaches the tip at one shade per two voxels, and
// running out halfway up is the thing that ramp's own note argues against. See
// the wheat branch in strandShade.
constexpr int STRAND_ROW_STEP = 1;
...
// ONE WEIGHT PER SHADE THE SWARD REACHES, and at a step of one that is the
// whole ramp. Written as an array rather than three named constants because
// the count is now derived -- kBladeRampReach moves with the step and with the
// grass height, and three hand-named entries could not follow it.
constexpr int bladeW(int k) { return bladeShadeWeight(k); }
constexpr int kBladeWTotal = bladeW(0) + bladeW(1) + bladeW(2) + bladeW(3) +
                             bladeW(4) + bladeW(5);
static_assert(mat::BGRASS_FLOOR_COUNT == 6,
              "the floor's weight table is written for six shades");
...
// blade at all.
// -----------------------------------------------------------------------
// THE SHADE A BLADE VOXEL ACTUALLY WEARS -- the host's copy of strandShade.
//
// (user 2026-09-20: "now the wheat turns green".)
//
// bladeMaterial RETURNS THE RAMP'S BASE, NOT A SHADE. Nothing on the terrain
// path needs more: the mesher writes that one id, the column carries a strand
// code beside it, and strandShade turns the pair into a per-voxel gradient ON
// THE DEVICE. So every query that asks the host what a stalk is made of gets
// the same answer for its foot and its ear.
//
// A LOOSE BODY HAS NO COLUMN AND NO STRAND CODE. Trace.cs.slang draws it with
//     (inst.kind == KIND_LOOSE) ? triMaterial(packed) : groundShade(...)
// -- the stored id, exactly -- which is right, because a piece that moves
// cannot go on hashing its world position. But it means a cut stalk wears its
// BASE for its whole length, and the base of a straw ramp is "this wood's
// green at the foot" (see fillWheatRamp). Cut wheat came away GREEN, all of
// it, and no amount of filtering which blades were cut would have changed
// that -- the grass in the tuft was a fifth of the voxels.
//
// SO THE SHADE IS RESOLVED WHERE THE COLUMN IS STILL IN HAND, which is the
// same argument meshVolume's `resolved` makes for the families it handles --
// and this is the one it does not, because a blade is a GRADIENT and not a
// hash. `lo` is bladeSpan's, which is what strandCodeFor encodes.
//
// THE HALVING BELONGS TO THE STRAW and is deliberately the shader's line
// again, not an approximation of it: ten shades over a twenty-voxel tuft
// would otherwise reach the tip halfway up and stand there.
// -----------------------------------------------------------------------
inline uint8_t bladeShadeFor(uint8_t base, int lo, int y) {
    int count = 0;
    if (base == mat::GRASS_0) count = mat::GRASS_COUNT;
    else if (base == mat::BGRASS_0) count = mat::BGRASS_COUNT;
    else if (base == mat::WHEAT_0) count = mat::WHEAT_COUNT;
    else if (base == mat::BWHEAT_0) count = mat::BWHEAT_COUNT;
    else return base;   // not a strand ramp: it has no rows to walk
    int row = (y - lo) / STRAND_ROW_STEP;
    if (base == mat::WHEAT_0 || base == mat::BWHEAT_0) row >>= 1;
    if (row < 0) row = 0;
    if (row > count - 1) row = count - 1;
    return uint8_t(int(base) + row);
}

inline uint8_t strandCodeFor(int baseRow) {
    // DIVIDED BY THE STEP, not by a literal two: the two were the same number
    // until the step became one, and a hard-coded shift here would have left
    // the host storing halves while the shader read wholes.
    //
    // MODULO STRAND_PHASES, NOT MASKED BY THE FIELD. See the note over
    // STRAND_PHASES: masking gave 1..32 and the field holds 0..31, so one row
    // in thirty-two packed onto the sentinel and lost its gradient.
    //
    // THROUGH uint32 ON PURPOSE, because the shader reads the row as a uint
    // and a negative row has to fold to the same phase on both sides. A
    // signed % would round toward zero here and the shader would not agree.
    return uint8_t(1 + (uint32_t(baseRow / STRAND_ROW_STEP) % uint32_t(STRAND_PHASES)));
}
...
class VoxelTerrain {
  public:
    ...
    // -- IS THIS COLUMN MOUNTAIN, as far as a crop is concerned ------------
    //
    // TWO QUESTIONS, because "mountain" is two things. High ground is the
    // timberline, which only exists under a DEM and costs one compare when it
    // does not. STEEP ground is the slope, and it is the one that answers in
    // the procedural world as well -- the same two-column drop kTreeSlope is
    // measured in, so a reader has one unit to hold.
    //
    // ASKED WITHOUT THE MEMO on the four neighbours on purpose: the memo
    // caches this column's own field state, and handing it another column's
    // coordinates would answer the wrong question and poison it for whatever
    // asks next. Four terrain evaluations is the price, and it is only paid
    // for a column that has already grown a blade AND landed inside a tuft
    // site -- see the caller in strandRows.
    bool mountainForWheat(int i, int j, TerrainMemo &memo) const {
        if (aboveTimberlineVox(heightVox(i, j, memo))) return true;
        const int dx = absi(heightVox(i + 1, j) - heightVox(i - 1, j));
        const int dz = absi(heightVox(i, j + 1) - heightVox(i, j - 1));
        return maxi(dx, dz) > kWheatSlope;
    }
    ...
    // How deep it lies where it lies at all, in WORLD metres.
    //
    // -- 0.8 -> 2.0 (user 2026-09-20: "we need added snow ontop of the
    //    mountain as well") ------------------------------------------------
    //
    // THE SNOW HAS RAISED THE COLUMN SINCE 2026-09-18 -- it is real geometry
    // and materialAt fills what the column grew by, not a paint. What was
    // wrong is that there was not enough of it to read as lying ON anything.
    // Measured round the highest summit in the window, (950, 1200) at 418
    // world m: 2207 columns carry snow and only 333 of them -- fifteen per
    // cent -- were over half a metre. Under five voxels on a band that is
    // ninety-six world metres tall is a skin, which is exactly what it looked
    // like.
    //
    // 2.0 m is twenty voxels. The original note's worry is still the right
    // one -- "thin enough that it follows the rock underneath instead of
    // burying it into a dome" -- and it survives this, because the couloirs
    // this lies in have tens of metres of relief and the grade terms below
    // still shed it off anything steep. It is a knob: this is the number to
    // move if it ever reads as a dome.
    static constexpr float kSnowDeepM = 2.0f;
    ...
    // -----------------------------------------------------------------------
    // -- WINTER, EVERYWHERE BUT THE SAND ---------------------------------
    //
    // (user 2026-09-20: "import the snow mechanics from v1. apply the snow to
    //  all biomes except the desert currently. give me a snow toggle in the
    //  settings".)
    //
    // THE SNOW THIS ENGINE ALREADY HAD IS AN ALTITUDE BAND ON A PHOTOGRAPH:
    // every line below asks the DEM, and the first one gives up without it --
    // so in the ordinary procedural wood there was no snow at any height, which
    // is why it reads as a feature that is not there rather than one that is
    // switched off.
    //
    // -- ...AND THEN THE BLANKET WAS THE WRONG HALF OF IT ---------------
    //
    // (user 2026-09-20: "stop making the terrain white when turning the snow
    //  on.")
    //
    // `snowOn` IS THE WEATHER NOW AND NOTHING ELSE. It was written as a
    // season -- every column white whatever its altitude -- because at the
    // time the only snow this engine could draw was on the ground. Now that
    // it FALLS (see flakeNearest in Trace.cs.slang), painting the world white
    // the instant it starts is the opposite of what was asked for twice: snow
    // is supposed to arrive from the sky and settle, and a blanket that is
    // already there is a blanket nothing settled into.
    //
    // SO THE TERRAIN NO LONGER READS THIS AT ALL. snowDepthAt and topMaterial
    // are back to the SNOWLINE they were: real altitude on a real DEM, which
    // is a fact about the mountain rather than about the weather, and the
    // peaks keep their white in --dem worlds exactly as before. The toggle
    // feeds V2Tracer::snowFall and stops there.
    //
    // THE ACCUMULATION IS THE MISSING PIECE and this is what makes room for
    // it: the ground has to start bare for anything landing on it to show.
    bool snowOn = false;

    // -- ...AND WHAT HAS SETTLED SO FAR ------------------------------
    //
    // (user 2026-09-20, the third time: "the snow should accumulate on the
    //  ground like in v1.")
    //
    // 0 IS BARE GROUND AND 1 IS THE FULL BLANKET. The old `snowOn` blanket
    // was this at 1 the instant the switch was flipped, which is what made it
    // read as paint; the ground now starts bare and this walks up while it
    // snows and back down when it stops.
    //
    // IT MOVES IN STEPS, AND THAT IS NOT A SHORTCUT. Snow depth is TERRAIN --
    // it raises the column, so every chunk that shows it has to be re-meshed
    // by workers holding their own copy of this generator (see
    // ChunkMesher::restart). A continuous depth would mean re-meshing the
    // world every frame. Steps of a voxel are what the ground can actually
    // express anyway: the blanket is kSnowDeepM at most, so there are only so
    // many heights it can be.
    //
    // NOT PER FLAKE. v1 mirrors the shader's lattice on the CPU and places a
    // voxel at the column and moment each flake lands. That is the better
    // answer and it is a much larger one -- it needs the landing sweep, the
    // settle-to-the-lowest-neighbour rule and the per-column melt. This is
    // the blanket arriving over a minute instead of instantly, which is the
    // half of it you can see from where you are standing.
    float snowLay = 0.0f;

    float snowDepthAt(float x, float z, float bareY) const {
        // A MOUNTAIN OR A STORM. The snowline is altitude and needs the DEM;
        // a blanket that has settled out of the weather does not.
        if (!dem_.ok() && snowLay <= 0.0f) return 0.0f;
        ...
        // THE ALTITUDE TERM IS THE SEASON'S TO OVERRIDE. Without a DEM there
        // is no "above sea level" to ask, and with the season on the answer is
        // one everywhere regardless -- so the band is only consulted when the
        // snow is the SNOWLINE'S rather than the winter's.
        // THE ALTITUDE BAND IS THE SNOWLINE'S and a settled blanket is not
        // subject to it: snow that fell out of the sky lies in the valley as
        // readily as on the ridge. Where there is no DEM there is no band to
        // ask, and the blanket is the whole of the answer.
        float t = 1.0f;
        if (dem_.ok()) {
            const float asl = dem_.worldToAsl(bareY);
            const float band = (asl - (kSnowlineAslM - kSnowFadeAslM)) / kSnowFadeAslM;
            t = maxf(minf(1.0f, band), snowLay);
            if (t <= 0.0f) return 0.0f;
        }
        // ...it slides off anything steep. Measured over 8 real metres, which is
        // the scale a slab actually fails on.
        // MEASURED OFF WHATEVER MAKES THE GROUND. With a DEM that is the
        // data; without one it is the height field itself, which is the same
        // question asked of the world that actually exists.
        const float sh = dem_.ok() ? dem_.shrink() : 1.0f;
        const float d = 4.0f / sh;
        float rise = 0.0f;
        if (dem_.ok()) {
            rise = maxf(fabsf(dem_.heightM(x + d, z) - dem_.heightM(x - d, z)),
                        fabsf(dem_.heightM(x, z + d) - dem_.heightM(x, z - d)));
        } else {
            TerrainMemo sm;
            const float h0 = heightM(x, z, sm);
            rise = maxf(fabsf(heightM(x + d, z, sm) - h0), fabsf(heightM(x, z + d, sm) - h0)) *
                   2.0f;
        }
        ...
        // ...AND ONLY AS MUCH AS HAS FALLEN. See snowLay.
        return t * kSnowDeepM * dry * snowLay;
    }
    float snowDepthM(float x, float z) const {
        // A MOUNTAIN OR A STORM. The snowline is altitude and needs the DEM;
        // a blanket that has settled out of the weather does not.
        if (!dem_.ok() && snowLay <= 0.0f) return 0.0f;
        if (dem_.ok()) return snowDepthAt(x, z, dem_.heightM(x, z));
        TerrainMemo sm;
        return snowDepthAt(x, z, heightM(x, z, sm));
    }
    ...
    // -----------------------------------------------------------------------
    // WOULD THE TREE SCATTER PLANT ANYTHING HERE?
    //
    // (user 2026-09-21: "when pressing g to teleport, try to avoid placing
    //  the player in sparse desert areas ... theres desert areas in the
    //  forest biomes. try avoiding those areas".)
    //
    // THOSE PATCHES ARE NOT A BUG IN THE TERRAIN, they are the imagery being
    // obeyed: coverGround is on, so a column the land-cover calls anything
    // but Forest grows no trees, and the same is true above the timberline.
    // Both are correct, and both make a perfectly good place to stand a poor
    // place to be DROPPED -- you arrive in a forest biome on bare ground with
    // nothing in sight.
    //
    // SO THE DESTINATION ASKS THE SCATTER'S OWN TWO GATES rather than a
    // threshold of its own. Copied predicates drift; these are the same two
    // calls, in the same order, that canopyNear and the scatter make before
    // they roll -- so "where trees are" and "where [G] puts you" cannot
    // disagree.
    //
    // NOT A TEST FOR THE DESERT BAND, which grows no trees anywhere and is a
    // destination the user explicitly kept. The caller asks this only when it
    // is aiming at a wood.
    // -----------------------------------------------------------------------
    bool plantableSpot(float x, float z) const {
        if (!coverAllowsTree(x, z)) return false;
        TerrainMemo memo;
        const int i = int(floorf(x / VOXEL_M)), j = int(floorf(z / VOXEL_M));
        return !treeRejectedByAltitude(i, j, heightVox(i, j, memo));
    }
    ...
    // -----------------------------------------------------------------------
    // IS THERE A CHERRY OVER THIS POINT? The one question "under the trees"
    // means, and the one both callers ask.
    //
    // (user 2026-09-21: "if the pink moss large rocks are not inside the tree
    //  line, remove the pink moss. or maybe move the pink moss rocks much
    //  closer into the cherry forest?")
    //
    // THE STAND FIELD WAS THE WRONG PROXY FOR BOTH OF THEM. It is a sixty-metre
    // fbm saying "this is thick wood", and both the petals and the pink stones
    // gated on `standDensity > kPetalStand` -- which is how a mossed boulder
    // ended up on open ground with no blossom in sight, twice reported. The
    // lattice is the authority: canopyNear replays the mesher's own tree sites,
    // so it answers about TREES rather than about the field the trees are
    // scattered from. See the note over canopyNear.
    //
    // ONE FUNCTION SO THE TWO CANNOT DRIFT. The scatter note already says the
    // petals and the moss have to agree about what "under the trees" means;
    // they agreed on the wrong test, and agreeing is only worth anything if it
    // is on the right one.
    bool underCanopy(float x, float z, TerrainMemo &memo) const {
        const int i = int(std::floor(x / VOXEL_M)), j = int(std::floor(z / VOXEL_M));
        const float fill = stemFill(x, z, heightVox(i, j, memo));
        return canopyNear(x, z, fill, memo);
    }
            ...
            for (int cx = cx0; cx <= cx1; ++cx) {
                ...
                for (int j = j0; j <= j1; ++j)
                    for (int i = i0; i <= i1; ++i) {
                        ...
                        // -- ...AND THE GATES THE SCATTER ALSO APPLIES -------
                        //
                        // (user 2026-09-21: "theres pink petals where there are
                        //  no trees", with a picture of a bare slope covered in
                        //  them.)
                        //
                        // THE NOTE OVER THIS LATTICE PREDICTED THIS EXACTLY:
                        // "leaving them out can only ever leave a pool of
                        // petals under a tree the scatter declined to plant on
                        // a steep bank". That is the photograph. The lattice
                        // mirrored WHERE a tree would be drawn and not WHETHER
                        // the ground would take one, so every site the scatter
                        // refused for slope, altitude, cover or water still
                        // dropped blossom.
                        //
                        // THEY ARE ASKED AT THE SITE, NOT AT THE PETAL, which
                        // is the whole point -- a petal on flat ground under a
                        // bank is fine, and a petal on the bank is not. Same
                        // four tests, same order, same constants as the scatter
                        // in VoxelChunks::scatter; a copy that drifts is worse
                        // than none, so they are written in the same sequence
                        // to be diffable by eye.
                        //
                        // WHAT IT COSTS, and why it is affordable now: five
                        // memoised height reads per candidate cell. This used
                        // to run on every cherry column that passed a stand
                        // test; it runs on the 10% that pass the petal odds,
                        // because that roll was moved ahead of it in the same
                        // batch that removed kPetalStand.
                        if (mappedWater(sx, sz)) continue;
                        const int slope = maxi(absi(heightVox(ci + 1, cj, memo) -
                                                    heightVox(ci - 1, cj, memo)),
                                               absi(heightVox(ci, cj + 1, memo) -
                                                    heightVox(ci, cj - 1, memo)));
                        if (slope >= kTreeSlope) continue;
                        if (treeRejectedByAltitude(ci, cj, heightVox(ci, cj, memo))) continue;
                        if (!coverAllowsTree(wx(ci), wx(cj))) continue;
                        if (outX) *outX = sx;
                        if (outZ) *outZ = sz;
                        return true;
                    }
...
// -----------------------------------------------------------------------
    ...
    // -- ...AND WHEAT IS A FIELD CROP ------------------------------------
    //
    // (user 2026-09-20: "prevent wheat from growing on mountains".)
    //
    // A TIGHTER LINE THAN THE TREE'S, and that is the point: a pine will take
    // a hillside a field never would. Derived from kTreeSlope rather than
    // written as its own number so the two cannot drift when the relief is
    // scaled again -- which the note above records happening twice already.
    static constexpr int kWheatSlope = kTreeSlope / 2;
    ...
    bool petalColumn(int i, int j, uint8_t top, TerrainMemo &memo, float *treeX,
        ...
        const float x = wx(i), z = wx(j);
        // -- ...AND NOT IN THE LAKE (user 2026-09-21: "theres pink petals in
        //    the water") -------------------------------------------------
        //
        // THE LINE ABOVE BELIEVED IT HAD ALREADY DONE THIS -- "a petal on bare
        // rock or in a LAKE is not a petal" -- and it had not. `top` is the
        // top SOLID material, and the top solid of a lake column is its BED,
        // which in the cherry wood is soil. So every submerged column passed
        // the floor test and grew a petal on the bottom of the lake.
        //
        // WET IS ITS OWN QUESTION, asked of the water line and never of the
        // ground: the same lesson waterY taught. wetColumn is the point path
        // the mesher itself uses, so a petal and a water voxel cannot disagree
        // about a column.
        const int h = heightVox(i, j, memo);
        // V2_PETAL_WET=0 puts the old behaviour back for one run, so "are there
        // still petals in the water" is an A/B on one camera rather than an
        // argument about what the code must be doing.
        static const bool dry = [] {
            const char *e = std::getenv("V2_PETAL_WET");
            return !(e && e[0] == '0');
        }();
        // AT OR BELOW THE LINE, NOT MERELY "WET". wetColumn is the mesher's
        // rule for where a water VOXEL goes, and it deliberately leaves the
        // fringe dry -- a column one voxel under the line with no deep
        // neighbour gets no water. That is right for the mesh and wrong here:
        // such a column is still a dip in the shallows, and a petal lying in
        // it reads as a petal floating in the lake. The waterline is the
        // honest edge for a thing that is supposed to be lying on dry ground.
        const int line = lakeLineAt(x, z, memo);
        if (dry && line != kNoWaterVox && h <= line) return false;
        // -- THE ODDS COME FIRST NOW, AND THAT IS A COST DECISION ---------
        //
        // canopyNear replays two dozen lattice cells; this hash is one
        // multiply. Asking it last meant paying for the lattice on every
        // column that then lost a 1-in-10 roll. Asked first it is the same
        // petals in the same places -- the hash is a function of the column
        // and nothing above it has changed -- for a tenth of the lattice work.
        if (hashUnit(0x9E7A13u, hashU32(uint32_t(i), uint32_t(j))) >= kPetalOdds) return false;
        // -- AND UNDER A TREE, WHICH IS NOW THE ONLY THING THAT DECIDES IT -
        //
        // (user 2026-09-21: "theres also no pink petals underneath the cherry
        //  trees consistently".)
        //
        // THE kPetalStand PRE-GATE IS GONE, AND IT WAS THE INCONSISTENCY.
        // It refused any column whose standDensity was under 0.40, on the
        // argument that it is a cheap way to reject most of the band before
        // paying for the lattice. But canopyNear ALREADY folds the stand field
        // in -- `sgate` -- and the acceptance it builds is
        // `(sgate^2 * gain + floorV) * density * fill`, with a floorV that is
        // not zero. A cherry can therefore stand anywhere the density is above
        // nothing at all, and every one of them below 0.40 was a tree with no
        // petals under it. There is no safe positive threshold, so there is no
        // threshold: the lattice is the authority about where a trunk is, and
        // it is the only thing asked.
        //
        ...
        // candidate cell.
        const float fill = stemFill(x, z, h);
        return canopyNear(x, z, fill, memo, treeX, treeZ);
    }
    ...
    uint8_t strandRows(int i, int j, uint8_t top, TerrainMemo &memo) const {
        ...
        // short.
        // -- ...AND A BROADLEAF FLOOR IS A BROADLEAF FLOOR --------------
        //
        // (user 2026-09-20: "can you fix the grass in the cherry forest. have
        //  the grass/grass strands match the birch forest".)
        //
        // THE DENSITY ASKED THE OAK AND THE HEIGHT DID NOT. Two lines up, `p`
        // is lerped to oakGrassDensity by `om`; these two were still lerped by
        // `bm` alone, so the oak -- and the cherry with it, since the cherry IS
        // the oak in every ground weight -- grew the BIRCH'S amount of grass at
        // the PINE'S height. Thick, and stubby, which is what "does not match
        // the birch" looks like from inside it.
        //
        // The sum rather than a second lerp: the two woods do not overlap in
        // any band, so where one is up the other is zero and this is just
        // "how broadleaf is this column" -- the same quantity the density
        // reaches in two steps. Clamped because a seam can carry a little of
        // both.
        const float broad = clampf(bm + om, 0.0f, 1.0f);
        float loF = lerpf(float(grassMinRows), float(birchGrassMinRows), broad);
        float hiF = lerpf(float(grassMaxRows), float(birchGrassMaxRows), broad);
        ...
        float tuftRad = 0.0f;
        float tuftWant = 0.0f;
        // NOT ON A MOUNTAIN -- see mountainForWheat. Inside the tuft test
        // rather than beside it so the four extra height lookups are paid only
        // where a tall blade was actually about to stand.
        if (tuftAt(x, z, &tuftRad, &tuftWant) && !mountainForWheat(i, j, memo)) {
        ...
        // CLAMPED TO THE PHASE COUNT, not to the field width: a blade as tall
        // as the wrap would put its tip back on its own root shade.
        return uint8_t(
            mini(STRAND_PHASES,
