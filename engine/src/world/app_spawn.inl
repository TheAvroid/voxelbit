...
// -----------------------------------------------------------------------------
    ...
    bool pickShoreAnchor(uint32_t seed, float *outX, float *outZ, uint8_t *outWood) const {
        ...
        // -- ALL FIVE AGAIN (user 2026-09-20: "you seem to spawn the player in
        //    the same spots ... on the initial spawn its always the same spot
        //    in the pine forest") -------------------------------------------
        //
        // THIS REVERSES "have the person spawn in the pine forest by default"
        // (2026-09-19), and it is worth saying out loud rather than leaving to
        // be discovered: that request set nW to 1 and this sets it back to 5.
        //
        // THE POSITION WAS NEVER THE PROBLEM. Measured over five unseeded
        // launches with --spawn-pick: (-2327, 1580), (-1602, 3313),
        // (2219, 3759), (1956, 230), (-2018, 2733) -- kilometres apart every
        // time. What repeated was the WOOD, because nW was 1, and one wood
        // looks like one place however far you move inside it.
        // -- THE PINE, ALWAYS (user 2026-09-21: "always have the initial
        //    spawn start in the pine forest") --------------------------
        //
        //    THIS IS THE THIRD TIME THIS LINE HAS MOVED and the history is
        //    the useful part. 2026-09-19 set it to 1 for the same ask;
        //    2026-09-20 set it back to 5 because "on the initial spawn its
        //    always the same spot in the pine forest"; this sets it to 1
        //    again, now with the reason that complaint did not have.
        //
        //    THE POSITION STILL ROLLS, AND THAT WAS ALWAYS THE ANSWER TO IT.
        //    Measured over five unseeded launches with --spawn-pick at nW=1:
        //    (-2327, 1580), (-1602, 3313), (2219, 3759), (1956, 230),
        //    (-2018, 2733) -- kilometres apart every time. What repeated was
        //    the WOOD, and the wood repeating is now what was asked for.
        //
        //    AND THE OTHER FOUR ARE ONE KEY AWAY. [G] walks pine -> birch ->
        //    cherry -> desert -> oak (see biomeNames, whose ROW ORDER is that
        //    cycle), so the first press leaves the pine. Nothing has become
        //    unreachable; the opening has stopped being a roll.
        int nW = 1;
        if (t.forced) {
    ...
    // WHERE THE GAME OPENS. One clearing in the pine, measured in game and
    ...
    void chooseSpawn() {
        ...
        // -- THE OPENING IS ONE PLACE NOW (user 2026-09-21: "spawn the player
        //    first in the -2131 234 -2073. spawn the player here upon opening
        //    the game") ------------------------------------------------
        //
        //    ABOVE THE ROLL, NOT INSTEAD OF IT. Everything below still exists
        //    and still runs for [G], for /locate and for --spawn N; this is
        //    the BOOT only, which is the one arrival the ask is about.
        //
        //    A NAMED PLACE STILL WINS. --acadia, --ouachita, --lake, --peak,
        //    --front and an explicit --cam-x/--cam-z all set camGiven or
        //    camPlace, and each of them means "take me THERE" -- a fixed
        //    opening that overrode them would make six flags silently do
        //    nothing. Same for a forced biome: --desert asks for sand, and
        //    this clearing is pine.
        //
        //    THE y IS NOT USED, for the reason every pinned spawn in this
        //    engine gives: placeOnGround stands the body on whatever ground it
        //    finds, which is the only height that survives an edit, a dig or a
        //    different DEM. 234 is where the ground happened to be when it was
        //    read.
    ...
    void nudgeOutOfSolids() {
        ...
        auto blocked = [&](float x, float z) {
            // -- ...AND NOT IN THE WATER EITHER -------------------------
            //
            // (user 2026-09-20: "prevent the player from spawning in rocks or
            //  water".)
            //
            // THE ROCK HALF OF THIS WAS FIXED AND THE WATER HALF WAS NEVER
            // ASKED. Everything below is about SOLIDS, so a spawn could land
            // in a lake and nothing here would object -- and the spawn picker
            // deliberately steers toward water now (it takes the first shore
            // in the band and rings a glade round it), so the one thing that
            // makes a wet spawn more likely is the same change that made
            // spawns interesting.
            //
            // wetColumnAt is this engine's ONE definition of wet -- the same
            // three calls WaterField::rebuild makes and the same question the
            // bunnies and the lily pads ask -- so the spawn cannot disagree
            // with the lake about where the lake is.
