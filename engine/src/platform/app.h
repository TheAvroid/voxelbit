...
struct Options {
    ...
    // -- THE COMPASS (user 2026-09-21) ---------------------------------
    //
    // "add a compass feature. put it in the top middle of the screen. v1 had
    //  this feature. let it be toggleable in the settings under visuals. make
    //  the compass color gold. like the fps."
    //
    // OFF BY DEFAULT (user 2026-09-21). It shipped on, on the reasoning that a
    // compass nobody finds is a compass nobody has -- but the strip sits in
    // the top middle of the screen, which is the one part of the frame a
    // player is always looking through. The settings row under Visuals turns
    ...
    std::string pines = asset("game/assets/foilage/pine9");
    std::string decor = asset("game/assets/decoration");
    ...
    std::string font = asset("game/3x3-pixel.otf");
    ...
    std::string soundDir = asset("game/sound");
    // --snow: winter, in every wood but the desert. See VoxelTerrain::snowOn,
    // and the toggle in the settings that turns it on without a relaunch.
    bool snow = false;
    float sfx = 1.0f;
    ...
    std::string axe = asset("game/assets/stone_tools/stone_axe.vox");
    std::string pick = asset("game/assets/stone_tools/stone_pick.vox");
    std::string shovel = asset("game/assets/stone_tools/stone_shovel.vox");
    std::string hoe = asset("game/assets/stone_tools/stone_hoe.vox");
    std::string bow = asset("game/assets/stone_tools/bow_arrow/bow/base.vox");
    std::string arrow = asset("game/assets/stone_tools/bow_arrow/arrow.vox");
    bool axeOn = true;
    ...
    std::string coverPath = asset("engine/assets/dem/rmnp50.vbcov");
    ...
    std::string paletteVoxOut = asset("game/assets/palette_v2.vox");
    ...
    std::string seeds = asset("game/assets/decoration/seeds.vox");
    ...
    std::string steak = asset("game/assets/food/meat/steak/base.vox");
    // THE TWO FRUIT, AND THEY ARE THE FILES THE TREES USE -- see the kit note
    // for why one model serves both the crown and the hand.
    std::string apple = asset("game/assets/decoration/fruit_apple.vox");
    std::string orange = asset("game/assets/decoration/fruit_orange.vox");
    ...
    std::string rifle = asset("game/assets/guns/assault_rifle/fire/00.vox");
    ...
    std::string rifleReload = asset("game/assets/guns/assault_rifle/reload");
    ...
    std::string pistol = asset("game/assets/guns/pistol/shoot/00.vox");
    std::string pistolReload = asset("game/assets/guns/pistol/reload");
    ...
    std::string bulb = asset("source/wip/technology/lightbulb.vox");
    ...
    int fireFrame = -1;
    // ...AND A SCRIPTED DISCOVERY, in the same family and for the same reason.
    // The banner is earned by the first arrow ever loosed, which is a draw, a
    // release and a shaft in flight -- none of which a headless run can do. On
    // frame N this calls unlockProjectile() itself, so what gets photographed
    // is the real banner on the real path and not a picture of one.
    int achFrame = -1;
    ...
    int hurtFrame = -1;
    // Empty the health bar on this frame -- see --die-frame. For shooting the
    // game over screen, which otherwise needs a live death to look at.
    int dieFrame = -1;
...
//
// (user 2026-09-20: "remember that the games colors are light green and red.
//  with gold accents".)
//
// The panel was slate and cyan, which is a developer's colour scheme -- it
// belonged to nothing on screen. These are the wood's: the light green is the
// sward's lit end rather than a fresh hue, the red is the hurt flash's, and the
// gold is kGold above, which is already the discovery banner's and the
// watermark's.
//
// -- GOLD LEADS NOW, AND RED IS STILL RESERVED --------------------------
//
// (user 2026-09-20: "instead of green ui theme, make it gold.")
//
// THE NAMES ARE KEPT AND THE COLOURS MOVED, which is the smaller change of
// the two available and the one that cannot miss a call site. kGreen is read
// in nine places across the panel -- the card edge, the titles, the rules, the
// slider grabs, the checkmarks, three hover states -- and renaming it would be
// nine chances to leave one behind and a diff nobody can read. What the theme
// IS lives here; what reads it does not care.
//
// TWO GOLDS, for the reason there were two greens: kGold is the discovery
// banner's and the watermark's and it is bright, which is right for a thing
// you are meant to look at once. A panel wears it on every row, so the lead is
// a shade below that and the dim one is a bronze rather than a grey -- an
// edge and a hover have to read as the same family as the accent, not as its
// absence.
//
// RED IS UNTOUCHED. It means something in this game -- it is what the screen
// does when you are hit -- so it is spent on the one row that destroys work
