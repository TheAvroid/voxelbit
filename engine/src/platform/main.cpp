...
void usage() {
    std::printf(
        ...
        "  --rec-test                with no window: press R and ctrl+R empty-handed and\n"
        "                            with the gun up, and print which one each starts\n"
...
bool parseLifeOpt(const std::string &a, int argc, char **argv, int &i, Options *o) {
    ...
    if (a == "--ach-frame") { argInt(argc, argv, i, &o->achFrame); return true; }
    if (a == "--snow") { o->snow = true; return true; }
    ...
    if (a == "--die-frame") { argInt(argc, argv, i, &o->dieFrame); return true; }
    ...
    // -----------------------------------------------------------------------
    if (a == "--acadia") {
        o->demPath = asset("engine/assets/dem/acadia10.vbdem");
        ...
        o->coverPath = asset("engine/assets/dem/acadia10.vbcov");
        o->coverGround = false;
        o->demScale = 1.0f;
        o->birch = true;
    ...
    //     lake.
    if (a == "--ouachita") {
        o->demPath = asset("engine/assets/dem/ouachita12.vbdem");
        o->coverPath = asset("engine/assets/dem/ouachita12.vbcov");
    ...
    if (a == "--peak") { o->camX = 10316.0f; o->camZ = 18445.0f; o->camPlace = true;
                         return true; }
    if (a == "--front") { o->demPath = asset("engine/assets/dem/front60.vbdem");
                          o->coverPath = asset("engine/assets/dem/front60.vbcov");
...
bool parse(int argc, char **argv, Options *o, bool *vulkan, bool *debugLayer) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--pal-probe") { if (i + 1 < argc) o->palProbe = argv[++i]; continue; }
        // DOES A HUNTER COME AT YOU AND COST YOU HEALTH -- see runBiteTest.
        if (a == "--bite-test") { o->biteTest = true; continue; }
        // Which animals the crosshair can actually find -- see runAimTest.
        if (a == "--aim-test") { o->aimTest = true; continue; }
        // Where the startup seconds actually go -- see v2::loadMark.
        if (a == "--load-trace") { o->loadTrace = true; continue; }
        // BEFORE THE CHAIN, like every flag added since the C1061 -- see the
        ...
        if (a == "--rec-test") { o->recTest = true; continue; }
        ...
        if (a == "--deathvalley" || a == "--death-valley") {
            o->demPath = asset("engine/assets/dem/deathvalley50.vbdem");
            o->coverPath = "";
            o->desertOnly = true;
            continue;
        }
...
int main(int argc, char **argv) {
    ...
    //
    // A crash that happens during startup is still a crash, and the handler
    // costs one system call to install. See platform/crashlog.h for what it
    // writes and why the WER record on its own was not enough to act on.
    v2::installCrashHandler();
    // PROVE IT WORKS, ON DEMAND. A crash reporter that has never reported a
    // crash is a guess; V2_CRASH_TEST=1 faults deliberately here and the run
    // should leave v2-crash.log and a .dmp behind and nothing else. It is an
    // env var rather than a flag so it cannot be reached by accident from the
    // launcher, and it costs one getenv.
    if (const char *ct = std::getenv("V2_CRASH_TEST"))
        if (ct[0] == '1') {
            std::printf("v2: V2_CRASH_TEST -- faulting on purpose\n");
            std::fflush(stdout);
            volatile int *p = nullptr;
            *p = 1;
        }
    v2::loadT0();   // start the stopwatch before anything else -- see loadtrace.h
    Options o;
    ...
    // "v2 -- pine forest (Falcor)" is three internal facts -- the engine
    // revision, the biome it opened on when it only had one, and the renderer
    // it links -- none of which is the product. The [background] tag stays:
    // that is a state of THIS window and the only way to tell two apart.
    c.windowDesc.title = o.background ? "voxelbit v1.4 [background]" : "voxelbit v1.4";
    ...
    // -----------------------------------------------------------------------
    // -- ...AND --spawn-pick IS ONE OF THEM (user 2026-09-21: "stop opening
    //    up the game on my screen") -- THE FOURTH REPORT OF THIS -----------
    //
    //    It prints a position, an openness and a yaw and then exits. There was
    //    never a frame worth looking at, and it was missing from this list for
    //    no reason but that nobody had added it: the gate is a hand-written
    ...
    // -- THE GPU'S OWN BLACK BOX, BEFORE THE DEVICE EXISTS --------------
    //
    // (user 2026-09-21: "when killing a peice of life my game crashes",
    //  with DXGI_ERROR_DEVICE_REMOVED.)
    //
    // A removed device is not a CPU fault, so crashlog.h cannot see it: the
    // GPU faults, the driver tears the device down, and the stack then names
    // whatever innocent call happened to be next (Gui::render, in the report
    // this was written for). DRED records what the GPU actually did.
    //
    // BEFORE ForestApp, WHICH CREATES THE DEVICE. The settings object only
    // affects devices made after it; called later, every field reads back
    // empty and that looks exactly like "DRED found nothing".
    //
    // --debug implies it, since anybody passing that is already hunting.
    {
        const char *e = std::getenv("V2_DRED");
        v2::enableDred(debugLayer || (e && e[0] == '1'));
        // ...and the slow one, only when asked by name.
        const char *g = std::getenv("V2_GBV");
        v2::enableGbv(g && g[0] == '1');
    }
    ...
    try {
        ...
        v2::noteDredDevice(
            app.getDevice()->getNativeHandle().as<ID3D12Device *>());
        // ...AND HANG THE REPORT ON EVERY DOOR OUT. Falcor does not always
        // throw -- see the note in dred.h -- so the catch below is the least
        // reliable of the three.
        v2::installDeviceRemovedReporter(
            (Falcor::getRuntimeDirectory() / "v2-crash.log").string());
        ...
        std::fprintf(stderr, "\nv2: %s\n", e.what());
        // ...AND IF THE DEVICE IS GONE, WHY. Silent for every other failure
        // -- it asks GetDeviceRemovedReason first. The log is the same file
        // crashlog.h appends to, so a bug report carries both halves.
        {
            const std::string log =
                (Falcor::getRuntimeDirectory() / "v2-crash.log").string();
            v2::reportDeviceRemoved(log.c_str());
        }
        return 1;
    }
}
