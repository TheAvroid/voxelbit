...
// -----------------------------------------------------------------------------
    ...
    void onLoad(Falcor::RenderContext *ctx) override {
        ...
        // -- OUR ICON, NOT NVIDIA'S (user 2026-09-21: "change the falcor logo
        //    for the voxelbit .exe icon") ---------------------------------
        //
        //    Falcor's SampleApp::onLoad sets data/framework/nvidia.ico on
        //    every app it hosts -- fair for a sample, wrong for a game with
        //    its own name on the window. Overwritten HERE rather than patched
        //    into the Falcor tree: the checkout is vendored and every local
        //    change to it is a line in PATCHES.md that has to be reapplied,
        //    and this needs neither.
        //
        //    THROUGH asset(), so it resolves in both trees -- game/logo.ico in
        //    the repo, data/game/logo.ico in a packaged install.
        //
        //    THIS IS THE WINDOW AND TASKBAR ICON. The icon Explorer draws on
        //    voxelbit.exe itself is a Win32 resource compiled into the binary
        //    and is a separate thing; the INSTALLER already carries logo.ico
        //    (see tools/voxelbit.iss, SetupIconFile).
        if (getWindow()) {
            const std::filesystem::path ico = v2::asset("game/logo.ico");
            std::error_code ec;
            if (std::filesystem::exists(ico, ec)) getWindow()->setWindowIcon(ico);
        }
        ...
        // WINTER, BEFORE THE FIRST COLUMN IS ASKED FOR. snowOn changes the
        // ground's height and its material, so it has to be set before any
        // chunk is built -- the settings toggle reloads the world for the same
        // reason. See VoxelTerrain::snowDepthAt.
        world_.terrain.snowOn = opt_.snow;
        ...
        v2::loadMark("onLoad entered");
        world_.pineDir = opt_.pines;
        world_.palProbe = opt_.palProbe;   // --pal-probe, see addFlyerModel
        ...
        v2::loadMark("models: read, mesh, BLAS, upload");
        if (opt_.loadTrace) world_.reportLoad();
        ...
        v2::loadMark("world: the resident ring");
        ...
        v2::loadMark("terrain, DEM and cover");
        makeCrosshair();
        makeVitalsPasses();   // the two bars and the death curtain
        v2::loadMark("screen passes built");
        ...
        if (ddgi_.init(getDevice(), Falcor::getRuntimeDirectory() / "shaders" / "v1")) {
        ...
        if (opt_.axeOn) {
            ...
            // -- DO NOT PREWARM THE BEHAVIOUR COLOURS HERE. IT BREAKS THEM. --
            //
            // Tried 2026-09-20 and REVERTED. The idea was to register the
            // spark, ember, smoke and glow colours before the flyer band, so
            // that Particles::load and Critters could still find them once the
            // table was full. It registered cleanly -- "4 of 4" -- and it
            // BROKE ALL FIVE: sparks went `materials 0/0/0`, tears and the
            // firefly's glow to material 0, where before they were 252/253/254.
            //
            // Measured both ways on one build: with the prewarm and the OLD
            // tolerance restored the materials were still 0, which is what
            // proves the prewarm and not the minting policy did it.
            //
            // The reason is Particles::one, which wants a material NOTHING
            // ELSE IS WEARING -- every one of those reports ends "1 is
            // private". Pre-minting the exact colour puts a second entry in
            // range of the candidate it was going to choose, so the candidate
            // stops being private and is rejected; with every candidate
            // rejected it returns mat::AIR. See [[v2-private-material]], which
            // says the same thing from the other direction: "exact"
            // registration does not give you a private material, and the
            // failure is pixel-for-pixel silent.
            //
            // -- AND UNDER EXACT MINTING, THE BEHAVIOUR COLOURS FIRST -------
            //
            // Only under exact minting. See World::reserveBehaviourMaterials
            // for why it fixes the particles there and BREAKS them here under
            // the old tolerance -- both measured, both `materials 0/0/0`, from
            // opposite causes. The block comment above this one is the record
            // of doing it unconditionally and regretting it.
            if (v2::World::exactModels()) {
                // -- THE LAST CANDIDATE, NOT THE FIRST ----------------------
                //
                // (user 2026-09-20: "the sparks are the wrong color. they need
                //  to be more yellow. dont change the color but fetch the old
                //  color".)
                //
                // MINE, AND THIS IS THE OLD COLOUR FETCHED RATHER THAN A NEW
                // ONE PICKED. `Particles::one` starts from `cand[n - 1]` and
                // only moves off it if the search finds something else free --
                // so the LAST entry is the authored default and always was.
                // Reserving cand[0] quietly moved the spark from {255,202,96}
                // to {255,208,112}: eight more blue, which is exactly the
                // yellow going out of it.
                //
                // kSparkRgb / kSparkRedRgb / kSmokeRgb are NOT the right source
                // either -- they are the repaint-FROM values, not what one()
                // chooses. The candidate table is the only honest one.
                static_assert(sizeof(v2::kSparkCand) / sizeof(v2::kSparkCand[0]) == 5,
                              "the spark candidate table changed length -- the default is its LAST "
                              "entry, see Particles::one");
                const std::array<uint8_t, 4> beh[] = {
                    {v2::kSparkCand[4][0], v2::kSparkCand[4][1], v2::kSparkCand[4][2], 255},
                    {v2::kSparkRedCand[4][0], v2::kSparkRedCand[4][1], v2::kSparkRedCand[4][2], 255},
                    {v2::kSmokeCand[4][0], v2::kSmokeCand[4][1], v2::kSmokeCand[4][2], 255},
                    {v2::kGlowRgb[0], v2::kGlowRgb[1], v2::kGlowRgb[2], 255},
                };
                world_.reserveBehaviourMaterials(beh, 4);
                std::printf("  reserve  spark %u  ember %u  smoke %u  glow %u  "
                            "(exact minting)\n",
                            unsigned(world_.behaviourMtl(0)), unsigned(world_.behaviourMtl(1)),
                            unsigned(world_.behaviourMtl(2)), unsigned(world_.behaviourMtl(3)));
            }
        ...
        if (opt_.butterflies > 0) {
            ...
            // -- ...AND A TABLE FOR EVERY MODEL THAT LOADED AFTER THE WORLD --
            //
            // The second of exactly two calls: World::build makes the first,
            // before a chunk is adopted, because a decor instance takes its
            // offset once and keeps it. This one catches the held kit and the
            // whole flyer band, which load hundreds of lines below the world
            // and whose instances are rewritten every frame anyway.
            //
            // NOT THE LAST CALL, AND THE COMMENT THAT SAID SO WAS WRONG.
            // The held kit's tools and all three foods load hundreds of lines
            // below this, in this same function, so this pass cannot be the
            // one that catches them -- see the top-up at the end of onLoad,
            // and what claiming otherwise cost ("when I eat the steak, the
            // voxels turn green"). This call exists for the band and the kit's
            // PREWARM; the one at the end exists for everything after.
            world_.assignModelTables();

            {
                ...
                std::printf("  palette  %d of %d entries used%s\n", used, int(mat::COUNT),
                            lost ? "  -- FULL" : "");
                // WHAT THE PER-MODEL TABLES COST, printed next to what they
                // are for. They are copies of the wood's today, so this is the
                // headroom that is about to exist rather than headroom that
                // does -- see World::assignModelTables.
                {
                    const uint32_t nt = world_.modelTableCount();
                    std::printf("  tables   %u per-model + the wood + the arcade  "
                                "(%.1f MB of material table)\n",
                                nt, double((2u + nt) * uint32_t(mat::COUNT)) * 32.0 /
                                        (1024.0 * 1024.0));
                    // HOW MANY COLOURS THE PRIVATE TABLES ACTUALLY RESCUED.
                    // This is the number the "PALETTE FULL" line above is
                    // about: each one is a colour the wood refused, which
                    // used to be worn as the nearest shade the wood owned
                    // and is now the authored one. See
                    // World::privateModelEntry.
                    std::printf("  private  %d colour(s) the wood refused, minted into "
                                "the model's own table instead of snapped\n",
                                world_.privateEntryCount());
                    std::printf("  private  %d of them reached the buffer, across %d "
                                "model(s) with a table of their own\n",
                                world_.privateLaidCount(), world_.privateModelCount());
                    std::printf("  private  %lld voxel(s) in the art wear one\n",
                                world_.privateVoxelCount());
                    // THE OAK SPLIT, COUNTED. 0 seen means the range test never
                    // matched (the index is reduced -- see makeInstance); seen
                    // but 0 dark means the hash is wrong. Different bugs.
                    if (world_.oakSeenCount())
                        std::printf("  oaks     %lld placed, %lld of them the dark variant "
                                    "(%.0f%%)\n",
                                    world_.oakSeenCount(), world_.oakDarkCount(),
                                    100.0 * double(world_.oakDarkCount()) /
                                        double(world_.oakSeenCount()));
                    // ...AND HOW MANY SURVIVED INTO THE TLAS ARRAY. 0 here while
                    // the line above is not means makeInstance wrote a field that
                    // never reached the buffer the shader actually reads.
                    std::printf("  oaks     %lld flagged instance(s) reached the TLAS\n",
                                world_.tlasTreeFlaggedCount());
                }
                if (lost)
                    // -- WHAT THIS LINE USED TO SAY WAS WRONG TWICE OVER ----------
                    //
                    // It said the colours "render as AIR". They have not for
                    // some time: Palette::forModelColor SNAPS on overflow --
                    // nearestModelColor(c, 1 << 15) -- precisely so a refused
                    // colour is a wrong shade you can SEE rather than a voxel
                    // you cannot. Believing this line cost a wrong diagnosis on
                    // 2026-09-20: it reads like missing geometry and it is not.
                    //
                    // And since the per-model tables landed it is wrong again:
                    // a model with a table of its own TAKES the colour instead
                    // of snapping, so some of these are not losses at all.
                    //
                    // `lost` is DISTINCT colours the wood refused; `calls` is
                    // how many times it was asked. They are different numbers
                    // and printing them as one was half the confusion.
                    std::fprintf(stderr,
                                 "v2: PALETTE FULL -- the wood refused %d distinct "
                                 "colour(s) across %d call(s); %d of those were taken "
                                 "into the model's OWN table instead. Any that were "
                                 "not are drawn as the nearest shade the wood owns, "
                                 "NOT as AIR. Models are served in load order.\n",
                                 lost, calls, world_.privateEntryCount());
            }
        ...
        v2::loadMark("life loaded and published");
        if (opt_.aimTest) {
            clock_.tday = opt_.timeOfDay;
            clock_.cycleSpeed = opt_.cycleSpeed;
            clock_.azimuthBase = opt_.sunAz;
        ...
        if (opt_.soundOn && !opt_.outGiven && !opt_.background && audio_.open()) {
            ...
            // -- AND THE FOOTSTEPS ------------------------------------
            //
            // v1's own level, ported as it stands: 0.372, which its note
            // records as 0.496 x 0.75 after "lower the grass footsteps by
            // 25%", read as AMPLITUDE. Deliberately not levelled with the
            // rest of the SFX -- v1 says the same, and the recording arrived
            // 56 LUFS down with 20 dB of headroom, so most of the gain it has
            // is all there was.
        ...
        if (opt_.axeOn) {
            ...
            // -- ...AND IT KEEPS ALL FOUR OF ITS CREAMS NOW ----------------
            //
            // (user 2026-09-21: "the hand held wheat is still not authoring
            //  the right colors. reload the hand held wheat color palete".)
            //
            // IT WAS FOLDING THEM ON PURPOSE. kRampMergeTol 24 collapsed the
            // wheat's four creams -- 255,243,149 down to 255,238,127, 22.6
            // apart end to end -- into ONE, so the ear was a flat cream and a
            // red rather than the gradient the artist drew. The reasoning is
            // in that constant's own note and it was sound at the time: the
            // wood's table was 250 of 255 full and those three entries were
            // the cheapest thing in it to give up.
            //
            // THAT BUDGET NO LONGER EXISTS. Per-object palettes landed after
            // it -- a colour the wood cannot fit is minted into the MODEL's
            // own table by privateModelEntry, and the held path takes that
            // route like everything else (4,677 private entries across 376
            // models at last count). So the four creams cost the wood nothing
            // it has to find: they either fit or they go private.
            //
            // THE STEAK KEEPS ITS OWN FOLD. Its note is about marbling a
            // player cannot distinguish, which is a different argument from
            // this one and still holds.
            ...
            if (!held_.addFood(world_, "steak", opt_.steak,
                               ...
                               HeldPose{8.799f, -1.624f, 8.730f, 0.628f, 0.750f, 1.920f, 1.000f},
            ...
            if (!held_.add(world_, "arrow", opt_.arrow,
                           // THE USER'S BAKE, 2026-09-20, off the [I] card. The three
                           // that moved are the arrow's own rotation -- it sat with
                           // the shaft across the view rather than along it.
                           HeldPose{8.799f, -0.910f, 8.730f, -1.606f, -1.326f, 1.641f, 1.000f},
                           Takes::Nothing))
                arrowTool_ = -1;
            ...
            // -- AND *NOW* EVERY MODEL HAS A TABLE. THE EARLIER CALL DOES NOT. --
            //
            // (user 2026-09-20: "when I eat the steak, the voxels turn green".)
            //
            // THAT WAS THIS. The call up beside prepareLevelPalette carries a
            // comment claiming it runs "after every loader in the program" and it
            // is WRONG: the held kit's tools and all three FOODS load hundreds of
            // lines below it, in this same function. Every model born after it
            // kept matBase 0, so World::uploadMaterials' layPrivate -- which skips
            // matBase == 0 -- never uploaded their private entries. The voxels
            // still carry those private ids, so the shader resolved them against
            // the WOOD's table instead of the model's own and drew whatever the
            // wood happens to keep at 250 and 251. On the steak that is a green.
            //
            // A BITE FRAME IS ITS OWN MODEL -- HeldItem::addFood calls addHeldVox
            // once per frame -- which is why it showed on EATING and not on
            // holding: the first frame shared the wood's entries and the later
            // ones had to mint.
            //
            // IT IS IDEMPOTENT, so this is a top-up rather than a rebuild, and it
            // stays a SECOND call rather than replacing the first: that one has to
            // happen before a chunk is adopted, for the reason
            // World::assignModelTables gives about decor.
            {
                const uint32_t was = world_.modelTableCount();
                const uint32_t now = world_.assignModelTables();
                // WHICH TABLE EACH HELD MODEL ENDED UP WITH. A 0 here is the
                // held-wheat bug: the entry is minted correctly and then read
                // out of the wood's table instead. See World::heldMatBase.
                if (std::getenv("V2_HELD_LOOK"))
                    for (int hi = 0; hi < world_.heldModelCount(); ++hi)
                        std::printf("           held model %2d  matBase %6u  %zu private%s\n",
                                    hi, world_.heldMatBase(hi), world_.heldPrivCount(hi),
                                    (world_.heldMatBase(hi) == 0u &&
                                     world_.heldPrivCount(hi) > 0)
                                        ? "   <== NO TABLE, private colours lost"
                                        : "");
                if (now != was)
                    std::printf("  tables   %u more for the loaders that run after the first pass "
                                "(%u total)\n", now - was, now);
                // -- AND WHAT THE KIT ITSELF MINTED ------------------------
                //
                // (user 2026-09-20: "eating the steak is causing green voxels
                //  to show".)
                //
                // The `private` lines above are printed BEFORE the kit loads,
                // so they have never once said whether a TOOL or a FOOD minted
                // a private colour. A held model with private entries and a
                // matBase of 0 reads them out of the wood's table, and the
                // wood's 250/251 is where the green came from last time.
                std::printf("  private  %d colour(s) total after the kit, %d reached the buffer "
                            "across %d model(s)\n",
                            world_.privateEntryCount(), world_.privateLaidCount(),
                            world_.privateModelCount());
            }
        ...
        // BESIDE THE HOE TEST AND FOR ITS REASON: this one needs the kit --
        // there is no gun to hold until the loader has run.
        if (opt_.recTest) {
            runRecTest();
            shutdown(0);
            return;
