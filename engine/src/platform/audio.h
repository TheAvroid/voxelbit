...
#include "player/dynamics.h"  // tameLoudPeaks
#include "platform/mfvideo.h"  // mfInit, widen, ComPtr -- and mfapi/mfidl/mfreadwrite

#include <xaudio2.h>

#include "platform/audiotap.h"
...
class Ambience {
  public:
    ...
    void update(float dt, float target) {
        ...
        // actually sounds.
        level_ += (target - level_) * (1.0f - std::exp(-fade_ * dt));
        ...
        voice_->SetVolume((square_ ? level_ * level_ : level_) * master_);
    }

    // -- ...AND TWO KNOBS THE FOOTSTEPS NEED -------------------------------
    //
    // (user 2026-09-20: "can you add the footstep sounds from v1 into v2. its
    //  the grass footstep sound".)
    //
    // A footfall loop is the same object as a bird bed -- one file, looping,
    // whose GAIN says whether you can hear it -- so it reuses this class
    // rather than growing a second one. Two things differ and both are here:
    //
    // THE FADE IS TWENTY TIMES FASTER. The bed's 0.45/s is tuned to a
    // sixty-metre stand field and lands a fade "a little behind the trees";
    // steps start and stop with a key, and v1 uses 0.09 s for the same reason
    // its note gives -- stopping dead clicks and starting dead drops you into
    // the middle of a footfall.
    //
    // AND THE SQUARING GOES. That taper is for a bed that is never in the
    // foreground; a footstep IS the foreground, and squaring a level that is
    // mostly 0 or 1 only softens the two ends of the ramp.
    void setFade(float perSec, bool square) {
        fade_ = perSec;
        square_ = square;
    }

    // THE STRIDE DOUBLES WHEN YOU SPRINT and the loop's seam stays where it
    // was -- v1's note: the cut is a property of the audio, not of the rate.
    // XAudio2's frequency ratio resamples, so the pitch rises with it; v1 sets
    // preservesPitch and cannot, which is the one audible difference between
    // the two engines here.
    void setRate(float r) {
        if (!voice_ || r <= 0.0f) return;
        if (r == rate_) return;
        rate_ = r;
        voice_->SetFrequencyRatio(r);
    }
  ...
  private:
    ...
    float level_ = 0.0f;
    float master_ = 1.0f;
    float fade_ = 0.45f;    // per second -- see setFade
    bool square_ = true;    // ...and whether the ramp is tapered
    float rate_ = 1.0f;
};
