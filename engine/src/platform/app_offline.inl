        ...
        if (flock_.ready()) {
            ...
            // their spawn and the fish are not all pointing the same way.
            {
                ...
                for (int mk = 0; mk < kMarchKinds; ++mk) {
                    ...
                    {
                        // HOW FAR APART THE NEAREST TWO STAND, WHICH IS NOT
                        // WHERE THEY WERE BORN. "nearest 32 m" is a distance to
                        // the PLAYER and says nothing about spacing, so this is
                        // the better of the two -- but it is measured after the
                        // marchers have walked, and a marcher walks. A flamingo
                        // pair born 1.11 m apart reads 5.25 m here two frames
                        // later, and that is the birds wandering, not a failed
                        // pairing: this number cannot tell those apart and was
                        // briefly believed to. V2_FLAM_PROBE=1 prints the pair
                        // AT BIRTH, which is the thing v1's FLAM_PAIR note is
                        // actually about. See Bunnies::marchPerCell.
                        float apart = 0.0f;
                        const bool two = bunnies_.closestTwo(mk, &apart);
                        std::printf("  %-8s %d marching, nearest %.0f m at "
                                    "(%.0f, %.0f, %.0f)%s%.2f m\n",
                                    kMarchSpec[mk].name, bunnies_.skunksLiving(mk),
                                    double(kd), double(kat.x), double(kat.y), double(kat.z),
                                    two ? ", closest two " : ", only one at ",
                                    two ? double(apart) : 0.0);
                    }
                    else
