...
class Tracer {
  public:
    ...
    void init(const Falcor::ref<Falcor::Device> &device, World *world, bool coopVec = false,
        ...
        dt.addShaderLibrary("v1/shaders/Trace.cs.slang").csEntry("main");
        ...
        du.addShaderLibrary("v1/shaders/Trace.cs.slang").csEntry("sharcUpdateMain");
        if (coopVec_ && device_->getType() == Falcor::Device::Type::Vulkan)
        ...
        if (nrcSdkOn_) {
            ...
            du2.addShaderLibrary("v1/shaders/Trace.cs.slang").csEntry("main");
            ...
            nrcSdkResolve_ = ComputePass::create(
                device_, "v1/shaders/NrcSdkResolve.cs.slang", "main", defs);
        }
#endif
        tonemap_ = ComputePass::create(device_, "v1/shaders/Tonemap.cs.slang", "main");
        ...
        demodulate_ = ComputePass::create(device_, "v1/shaders/Demodulate.cs.slang", "main");
        remodulate_ = ComputePass::create(device_, "v1/shaders/Remodulate.cs.slang", "main");
        ...
        probeTrace_ = ComputePass::create(device_, "v1/shaders/ProbeTrace.cs.slang", "main");
    }
    ...
    // -- THE WEATHER, DRIVEN FROM OUTSIDE --------------------------------
    //
    // snowFall is how dense the flake lattice is, 0 to 1; snowTime is the
    // clock it falls on. Both are set by the app each frame and handed
    // straight to V6Params -- see the note on them there.
    //
    // WHY THE TRACER HOLDS THEM AT ALL, rather than the terrain: falling
    // snow is drawn and nothing else. It is not in the acceleration
    // structure, it does not collide, the walk cannot feel it, and the
    // mesher has never heard of it. The BLANKET on the ground is the
    // terrain's (VoxelTerrain::snowOn) and the two meet only in the settings
    // row that turns both on.
    float snowFall = 0.0f;
    float snowTime = 0.0f;
    // How long the last frame was -- the flakes' motion vector needs it.
    float snowStep = 1.0f / 60.0f;
    ...
    // and decor paths use.
    std::string moonPath = asset("game/assets/moon.png");
    ...
    // -- THERE IS NO "RESET JUST THIS ONE THING" HERE, AND THERE WAS --------
    //
    // A `resetHistoryNextFrame()` sat here for part of a day, added to chase
    // the steak's residue and called on the frame the held MODEL changed. It
    // set the same `resetHistory_` below that a resize and a teleport set, so
    // it threw away the history for EVERY PIXEL to fix an artefact on a few
    // hundred of them -- and a held model changes on every step of a bow's
    // draw, a gun's reload strip and a food's bite frames. The result was
    // reported as "noise on the entire screen when pulling the bow back or
    // switching weapons", which is exactly what an un-denoised frame is.
    //
    // Both the method and its caller are gone. A local artefact needs a local
    // answer: V6Params::heldPrevValid for the item's own motion vector, or the
    // guide buffers per pixel. Nothing on the held item is worth this flag.
    ...
    void renderSample(Falcor::RenderContext *ctx, const V6Camera &cam, const RenderSettings &cfg) {
        ...
        // THE WEATHER -- see V6Params::snowTime. The clock is the render's own
        // seconds rather than the sim's, because a flake's fall is a LOOK and
        // should not stop when the world is paused.
        p.snowTime = snowTime;
        p.snowFall = snowFall;
        p.snowStep = snowStep;
