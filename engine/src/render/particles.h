...
class Particles {
  public:
    ...
    bool load(World &world, Tracer *tracer) {
        // Slots 0/1/2 are the spark, the ember and the smoke, in the order
        // app_load reserves them. -1 everywhere would be the old behaviour.
        spark_ = one(world, kSparkCand, 5, &sparkMtl_, &sparkShared_, sparkRgb_, 0);
        red_ = one(world, kSparkRedCand, 5, &redMtl_, &redShared_, nullptr, 1);
        smoke_ = one(world, kSmokeCand, 5, &smokeMtl_, &smokeShared_, nullptr, 2);
  ...
  private:
    ...
    // Critters::load.
    // `reservedSlot` indexes World::behaviourMtl -- see the branch below. -1
    // is "nothing was reserved", which is every call under the old tolerance.
    static int one(World &world, const uint8_t (*cand)[3], int n, uint8_t *mtl, int *shared,
                   uint8_t *chosen = nullptr, int reservedSlot = -1) {
        const uint8_t *rgb = cand[n - 1];
        uint8_t found[3] = {rgb[0], rgb[1], rgb[2]};
        // -- A RESERVED COLOUR SKIPS THE SEARCH, AND HAS TO -----------------
        //
        // The model below is built from `found`, and `*mtl` is what the shader
        // is told the emitter id is. Those must be THE SAME COLOUR. With a
        // reservation in hand the id is fixed (World::reserveBehaviourMaterials
        // minted cand[n-1] before the band), so letting the search wander to
        // some other candidate would build the voxel out of a colour that
        // resolves to a DIFFERENT id -- and the spark would quietly stop being
        // an emitter. cand[0] and cand[n-1] are about 17 apart here, outside
        // kModelMatch, so that is not a hypothetical.
        //
        // `got` starts true: cand[n-1] is already in `found` and is exactly
        // what was reserved.
        bool got = world.behaviourMtl(reservedSlot) != mat::AIR;
        ...
        int sx = 0, sy = 0, sz = 0;
        // -- MINTED EXACTLY, NEVER SHARED -----------------------------
        //
        // These are one-voxel models whose whole purpose is to OWN an id: the
        // spark's reaches the shader as an emitter and the smoke's as its own
        // surface, and the report below states the contract -- "1 is private".
        //
        // addFlyerModel folds at kLifeMatch by default, which is right for an
        // animal and catastrophic here, in the quietest possible way. When that
        // default arrived these three colours folded onto entries 28 away; the
        // resolve below then looked for them at tolerance 16, found nothing,
        // and every particle in the engine came back material 0 -- AIR. The
        // console said `materials 0/0/0` and nothing else went wrong.
        const int id = world.addFlyerModel(mo, "particle", &sx, &sy, &sz,
                                           /*keepVoxels=*/false, /*matchTol=*/0);
        if (id < 0) return -1;
        ...
        // -- ...UNLESS THE COLOUR WAS RESERVED, IN WHICH CASE ASK NOBODY ---
        //
        // Under exact-per-model the wood is FULL by the time particles load,
        // so this colour is refused into the particle model's own private
        // table -- and a lookup that must return a GLOBAL id cannot see a
        // private one. Measured `materials 0/0/0`, the same reading the note
        // above records from the folding bug, by a different route.
        //
        // World::reserveBehaviourMaterials mints it before the band instead,
        // and the id it got is the answer. No search, so nothing about how
        // full the table has become can change it.
        //
        // `shared` is 1 because under exact minting an id is worn only by a
        // BYTE-IDENTICAL colour, which is what private has to mean here.
        const uint8_t res = world.behaviourMtl(reservedSlot);
        if (res != mat::AIR) {
            *mtl = res;
            if (shared) *shared = 1;
        } else {
