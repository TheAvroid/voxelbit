...
class Bees {
  public:
    ...
    void update(float dt, const Vec3 &player, const std::vector<Vec3> &hives,
        ...
        for (size_t i = 0; i < bees_.size(); ++i) {
            ...
            step(&bees_[i], uint32_t(i), dt, blooms);
            // THE SAME EXPRESSION put() DRAWS WITH, or the two describe
            // different facings and the vector is worse than none.
            Bee &b = bees_[i];
            const float th = atan2f(b.vx, b.vz) + 3.14159265f;
            const float d = b.thHave ? th - b.thPrev : 0.0f;
            b.dth = atan2f(sinf(d), cosf(d));
            b.thPrev = th;
            b.thHave = true;
        }
    }
  ...
  private:
    ...
    struct Bee {
        ...
        // -- THE HEADING IT DREW LAST FRAME, AND THIS FRAME'S TURN ----------
        //
        // A bee's facing is DERIVED from its velocity at publish time rather
        // than stored, so there is nothing to difference unless it is kept.
        // place() carries only the box centre's travel; a bee that swings round
        // a bloom turns most of itself and moves its centre very little.
        float thPrev = 0.0f, dth = 0.0f;
        bool thHave = false;
