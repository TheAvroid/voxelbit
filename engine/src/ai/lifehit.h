...
// -- 0.16, AND DOUBLING IT WAS TRIED AND REVERTED -------------------------
//
// (user 2026-09-20: "I am clearly hitting the life, but it is not picking up".)
//
// MEASURED FIRST, with --aim-test, which points at the box centre of every live
// animal from 2.5 m: EVERY land animal already answered. The geometry is not
// what is missing, so widening it was a guess -- and the guess measured worse.
// At 0.32 the misses went 22 -> 34 of 132, because a fatter box on a NEIGHBOUR
// intercepts the ray first in a school: the fish and the ducklings started
// answering for each other.
//
// What --aim-test does say is that two classes are unreachable and both are
// correct: a songbird perched 13-31 m up a trunk is past kLifeReach3dM, and
// overlapping school members resolve to whichever box the ray meets first.
...
inline LifeKind lifeAtSlot(int slot) {
    ...
    if (s < kLakeSlots) {
        ...
        //
        // (user 2026-09-21: "double check and make sure the betta fish can be
        //  killed like the other life. dont drop steak though".)
        //
        // AND WITHOUT THIS RUN IT WAS NOT KILLABLE AT ALL -- which is the
        // second half of what this note is for. `fish` above is a sum of the
        // six species that existed when it was written; the betta was added to
        // fish_ and to the band and NOT to that sum, so its ten slots fell
        // through into the lily pads' run and answered "not alive". Everything
        // after it -- pads, dragonflies, ducks -- was being read ten slots out
        // of register at the same time.
        //
        // THE NOTE OVER THIS TABLE SAYS WHY THAT IS THE SHAPE OF THE BUG: it
        // is the band's layout read BACK, so a population that grows has to
        // move this with it, and nothing checks that it did.
        //
        // `meat` FALSE IS THE ASK. It is a two-inch ornamental fish; a steak
        // off one is the joke v1 avoided by not putting it in the water.
        if (s < kBettaCount) return {"betta", false, false};
        s -= kBettaCount;
