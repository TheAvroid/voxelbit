...
// -----------------------------------------------------------------------------
    static const std::vector<BiomeName> &biomeNames() {
        static const std::vector<BiomeName> t = {
            // -- THIS ORDER IS [G]'s ORDER (user 2026-09-21: "pine forest,
            //    then birch forest, then cherry forest, then desert, then oak
            //    forest") -------------------------------------------------
            //
            //    respawnToNextBiome walks this list, so the row order IS the
            //    cycle. It used to be the order the bands were added in --
            //    pine, birch, oak, cherry, desert -- which was a changelog,
            //    not a route. The oak moved to the end; nothing else did.
            //
            //    NOTHING ELSE DEPENDS ON IT. The console builds a completion
            //    pool from this and the tests iterate all of it; neither reads
            //    a position. The BAND layout -- which wood is physically next
            //    to which -- is VoxelTerrain's and is untouched, so /locate
            //    still walks whatever distance the ground actually puts there.
            {"pine", "pine_forest", Biome::Pine},
            {"birch", "birch_forest", Biome::Birch},
            {"cherry", "cherry_forest", Biome::Cherry},
            // The only entry here that is not a forest.
            {"desert", "sand", Biome::Desert},
            // v1's wood, imported 2026-09-16. Its band sits between the pine
            // and the birch -- see VoxelTerrain's band note -- so arriving
            // here last is a cycle order, not a walk.
    ...
    enum class Life {
        ...
        Salmon, Bass, Koi, Minnow, Catfish, Bluegill, Betta, Duck, Dragonfly, LilyPad,
    ...
    static const std::vector<LifeName> &lifeNames() {
        ...
        static const std::vector<LifeName> t = {
            ...
            // THE BLOSSOM'S OWN FISH, and the one water row with a wood -- it
            // is the cherry's and nowhere else, so /locate betta in a pine
            // wood should say so rather than walk you to a salmon.
            {"betta",     "",          Life::Betta,      2.0f, kWoodCherry, true},
    ...
    bool nearestLife(Life k, Vec3 *at) const {
        ...
        switch (k) {
            ...
            case Life::Betta:     return lake_.nearestFish(pos_, 6, at, &d);
    ...
    void respawnToNextBiome() {
        ...
        // -- ANYWHERE IN THE BAND, NOT ON ITS NEAREST EDGE ---------------
        //
        // nearestBandX returns the closest repeat of the tiling, which is the
        // SAME x every time you reach a given wood -- so the hop roamed in z
        // and stood still in x, and the five biomes read as five fixed places.
        // The band is 800 m wide; landing anywhere across it, on the press
        // count's own stream, is what makes a second visit a different spot.
        //
        // The salt differs from the z stride's so the two axes cannot move
        // together and turn the roam into a diagonal line.
        const float across =
            (hashUnit(0x5A17u + respawnHops_ * 2246822519u, 1u) - 0.5f) * VoxelTerrain::kBandW * 0.8f;
        float tx = nearestBandX(bn.biome) + across, tz = pos_.z + stride;

        // -- THE PINE HAS ADDRESSES, AND THEY ARE NOT ROLLED --------------
        //
        // (user 2026-09-21: "save the position -2135 231 -2077 for the pine
        //  spawnpoint. have this be the only spawnpoint when pressing g to
        //  cycle through biomes", then "add in 1942 120 2505 into the pine
        //  forest rotation. so the pine forest should have 2 locations now
        //  that it should cycle through. facing northwest".)
        //
        // Every other band keeps the roam above -- a second visit to the oak
        // is a different piece of oak, which is the whole point of the hash
        // streams. The pine is PLACES instead: two known clearings, taken in
        // turn, so either can be returned to and compared against.
        //
        // ONE ROW PER VISIT TO THIS BAND, not per press -- see pinnedVisit_,
        // which explains why the press count cannot be the cycle index.
        //
        // MEASURED, NOT ASSUMED: woodWeights is pine 1.000 with every other
        // band at zero at both x = -2135 and x = 1942, so neither is near a
        // seam and neither can land you in the neighbouring wood. The y in
        // each ask is not used -- teleportTo puts the body on the ground it
        // finds, which is the only height that can be right after an edit or
        // a different DEM.
        //
        // IT SKIPS THE RE-ROLLS BELOW TOO. Those exist to find somewhere dry
        // and wooded when the destination was chosen at random; a spot chosen
        // by hand has already answered both, and re-rolling it would throw
        // the address away the first time the loop disliked something.
        const PinnedSpawn *pinned = nullptr;
        int nPinned = 0;
        for (const PinnedSpawn &ps : kPinnedSpawns)
            if (ps.biome == bn.biome) ++nPinned;
        if (nPinned > 0) {
            const size_t band = size_t(bn.biome);
            const uint32_t turn = pinnedVisit_[band]++ % uint32_t(nPinned);
            uint32_t seen = 0;
            for (const PinnedSpawn &ps : kPinnedSpawns)
                if (ps.biome == bn.biome && seen++ == turn) { pinned = &ps; break; }
            tx = pinned->x;
            tz = pinned->z;
        }
        // -- ...AND IT IS RE-ROLLED UNTIL IT IS DRY ------------------------
        //
        // (user 2026-09-20: "when pressing g, its still spawning the player in
        //  water. prevent this from happening.")
        //
        // teleportTo ALREADY STEPS CLEAR OF WATER and that is still the
        // backstop, but it is a 40 m ring: on a lake wider than that every ring
        // is wet, the search quietly gives up, and the body is placed where it
        // was going anyway. A shoreline is easy to step off; the middle of open
        // water is not.
        //
        // So the DESTINATION is chosen dry instead, which is the cheap half of
        // the problem -- wetColumnAt is a pure terrain query with no chunk
        // behind it, so a re-roll costs nothing like a teleport does. Each
        // attempt walks the same two hash streams one step further, so the
        // result is still a pure function of how many times [G] has been
        // pressed and two players still land together.
        // -- ...AND NOT ONTO BARE GROUND EITHER -------------------------
        //
        // (user 2026-09-21: "try to avoid placing the player in sparse desert
        //  areas. the desert itself is fine, but not the forest biomes.
        //  theres desert areas in the forest biomes".)
        //
        // Same loop, same budget, one more predicate. Those patches are the
        // land cover being obeyed -- see VoxelTerrain::plantableSpot, which
        // asks the tree scatter's own two gates rather than inventing a
        // threshold, so a spot this accepts is a spot trees are allowed in.
        //
        // THE SAND IS EXEMPT AND THAT IS THE ASK IN ONE LINE: "the desert
        // itself is fine". A desert destination grows no trees anywhere, so
        // asking would reject all twenty rolls and land you wherever the
        // last one fell -- worse than not asking. Only a WOOD is held to it.
        const bool wantWood = bn.biome != Biome::Desert;
        auto poorSpot = [&](float x, float z) {
            if (wetColumnAt(x, z)) return true;
            return wantWood && !world_.terrain.plantableSpot(x, z);
        };
        for (int tryN = 1; !pinned && tryN <= kRespawnDryTries && poorSpot(tx, tz);
             ++tryN) {
            const uint32_t h = respawnHops_ * 2654435761u + uint32_t(tryN) * 40503u;
            tx = nearestBandX(bn.biome) +
                 (hashUnit(0x5A17u + h, 1u) - 0.5f) * VoxelTerrain::kBandW * 0.8f;
            tz = pos_.z + (hashUnit(0x9E37u + h, 0u) - 0.5f) * 2.0f * kRespawnRoamM;
        }
        bool stand = false;
        ...
        teleportTo(tx, tz, /*dryLand=*/true);
        // WHICH WAY YOU ARE POINTING WHEN YOU GET THERE. teleportTo moves the
        // body and leaves the camera alone, which is right for a roll -- there
        // is nothing at a random destination to be aimed at. A PINNED row is
        // the other case: it is a composed view, and arriving in the right
        // clearing facing the wrong way is not arriving at it. Only rows that
        // asked for a bearing get one; the rest keep the roll's behaviour.
        if (pinned && pinned->yaw != kKeepFacing) yaw_ = pinned->yaw;
    ...
    // `dryLand` steps the arrival clear of water. OFF by default, and that is
    // the point: most callers are aiming AT something, and half the life in
    // this engine lives in a lake. /locate fish, bass, koi, duck and lily all
    // want the water's edge, and a blanket step put the player 8 m inland with
    // the fish out of reach -- measured, --kill-test: "no aim: it is 5.5 m off
    // (-2.8 m up)" on every water row at once.
    //
    // [G] is the one caller with no target: it asks for a BIOME, so anywhere
    // dry in that band will do. See respawnToNextBiome.
    void teleportTo(float x, float z, bool dryLand = false) {
        ...
        // -- THE GROUND ARRIVES BEFORE THE PLAYER DOES --------------------
        //
        // (user 2026-09-20: "when pressing g to respawn in another biome, can
        //  you load the terrain first before teleporting the player".)
        //
        // A hop crosses kilometres, so NONE of the destination's chunks are
        // resident: the old order put the body there and let the streamer
        // catch up, which is a second or two of standing in an empty sky while
        // the wood builds itself underneath. primeBlocking is the same call
        // the first load uses to fill the ring before the first frame -- it is
        // exactly this problem, already solved, just never asked here.
        //
        // BEFORE THE WATER TEST AND THE COLLIDER GATHER, both of which ask the
        // world about the destination: the water test reads the terrain (which
        // is generated, so it would answer anyway) but collidersNear reads
        // ADOPTED CHUNKS, and before this call there are none there -- which
        // is the same blindness that used to drop a spawn inside a boulder.
        // -- REVERTED 2026-09-20: THIS PRIME KILLED THE DEVICE -------------
        //
        // (user: "when pressing g to teleport its crashing my game: can you
        //  revert the 'load the world in before teleportation' from earlier".)
        //
        // The stack is unambiguous and it is not a C++ fault:
        //
        //   DXGI_ERROR_DEVICE_REMOVED
        //     RtAccelerationStructurePostBuildInfoPool::getElement
        //     World::finishCompact          world.h:13685
        //     World::drainCompactions       world.h:13861
        //     World::update                 world.h:11732
        //
        // The GPU died reading back a COMPACTION query. primeBudgeted adopts a
        // whole ring's worth of chunks in one press, each one a BLAS build with
        // a post-build query behind it, and draining that backlog on the next
        // frame is what took the device out.
        //
        // WHAT REVERTING COSTS, said plainly because it is a real cost: the
        // note this replaced is right that collidersNear reads ADOPTED chunks,
        // and a hop crosses kilometres so none of the destination's are
        // resident yet. Without the prime the streamer catches up over a second
        // or two -- you land in open sky and the wood builds underneath -- and
        // the boulder test that gate exists for is blind again. That is a
        // second of ugliness against a crash, and the crash wins.
        //
        // The right fix is to SPREAD the adopt across frames rather than spend
        // it all in the press, which is the same shape as the shatter cost in
        // [[v2-shatter-cost]]. Not attempted here: the ask was to revert.

        // -- AND NOT INTO THE LAKE ----------------------------------------
        //
        // (user 2026-09-20: "still spawning in water when pressing g to
        //  respawn in another biome".)
        //
        // THE LAST FIX WENT INTO THE WRONG FILE. A water gate was added to the
        // spawn nudge in app_spawn.inl, which is the run's FIRST placement and
        // the only one [G] never uses: respawnToNextBiome lands through
        // teleportTo, and nothing on this path had ever asked about water at
        // all. Every /locate hop shares it, so all of them were exposed.
        //
        // A RING, NOT A REFUSAL, because the destination is chosen for what is
        // there -- a biome, a lake, an animal -- and refusing it outright would
        // mean [G] silently doing nothing. Stepping to the nearest dry ground
        // keeps the trip and drops only the puddle.
        //
        // wetColumnAt is the engine's one definition of wet -- the same three
        // calls WaterField::rebuild makes -- so this cannot disagree with the
        // lake about where the lake is.
        if (dryLand && wetColumnAt(x, z)) {
            bool moved = false;
            // WIDER THAN IT WAS. Ten rings of 4 m reached 40 m, which a lake
            // clears easily -- and when every ring is wet this loop simply
            // ends and the body is placed in the water. The destination is
            // now chosen dry (see respawnToNextBiome), so this is the
            // backstop for the cases that do not go through it -- and it says
            // so when it fails instead of failing quietly.
            for (int ring = 1; ring <= 30 && !moved; ++ring) {
                const float rad = 4.0f * float(ring);
                for (int step = 0; step < 16; ++step) {
                    const float a = 6.2831853f * float(step) / 16.0f;
                    const float nx = x + rad * cosf(a), nz = z + rad * sinf(a);
                    if (wetColumnAt(nx, nz)) continue;
                    x = nx;
                    z = nz;
                    moved = true;
                    std::printf("  locate   stepped %.0f m clear of the water\n", double(rad));
                    std::fflush(stdout);
                    break;
                }
            }
            if (!moved) {
                std::printf("  locate   still in the water after 120 m of ring -- "
                            "nowhere dry near here\n");
                std::fflush(stdout);
            }
            // The solids were gathered round the OLD point; a step of up to
            // 40 m needs them re-gathered or findClear is blind again -- which
            // is the bug the note above this one is about.
            if (moved) {
                TerrainMemo tm2;
                const int gi2 = int(std::floor(x / VOXEL_M)), gj2 = int(std::floor(z / VOXEL_M));
                const float gy2 = float(world_.terrain.heightVox(gi2, gj2, tm2) + 1) * VOXEL_M;
                world_.collidersNear(Vec3(x, gy2, z), 12.0f, &solids_);
            }
        }
        WalkWorld w;
