...
REM   * Everything BUILT lands under v2\build: v1.exe, Falcor.dll, slang, the
...
REM A running v1.exe cannot be overwritten, and the failure arrives at the very
REM END of the build as a bare "LNK1104: cannot open file v1.exe", which says
...
tasklist /FI "IMAGENAME eq v1.exe" 2>nul | find /I "v1.exe" >nul
if not errorlevel 1 (
  echo.
  echo v1: v1.exe is still running, and the linker cannot overwrite a running exe.
...
if not exist "%FALCOR_DIR%\CMakeLists.txt" (
  echo v1: no Falcor checkout at %FALCOR_DIR%.
...
if not exist "%CMAKE%" (
  echo v1: Falcor's dependencies are not fetched yet.
...
if not exist "%OUT%\CMakeCache.txt" (
  echo   cmake configure ^(Falcor + v1^)
  ...
  REM looking for "external/slang/c:/voxelbit/engine/lib". Typing it STRING seeds
  ...
  if errorlevel 1 ( echo v1: configure failed. & exit /b 1 )
)

REM --- build ------------------------------------------------------------------
echo   ninja %CONFIG%
"%CMAKE%" --build "%OUT%" --config %CONFIG% --target v1
if errorlevel 1 (
  echo.
  echo v1: build failed -- nothing was changed. The previous build still runs.
  echo     If the link failed, close any running v1.exe and build again.
  exit /b 1
)
...
for %%F in ("%OUT%\bin\%CONFIG%\v1.exe") do echo built %%~fF  ^(written %%~tF^)

REM --- ...AND THE SHIPPED EXE IS NOW A BUILD BEHIND ---------------------------
REM
REM dist\voxelbit.exe is a SNAPSHOT of a past build, not a link to this one.
REM A stale one still launches and still plays, which is what makes it dangerous:
REM the bug you just fixed is still in it, and the hour goes into chasing a ghost.
REM
REM This does NOT repackage. Packaging reads 530 MB, writes 780 MB and takes
REM minutes, against a relink of a few seconds; hanging it off every build would
REM make a one-line change cost a full package. It prints one line instead, and
REM only when there is something to say -- no dist means nobody is shipping yet.
REM See tools/package.py --check, and the rule in CLAUDE.md that says when the
REM exe must actually be refreshed.
REM ...AND IT IS AT THE ROOT, NOT IN dist\. package.py writes OUT_SETUP to
