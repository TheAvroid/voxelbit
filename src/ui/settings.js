  ...
  // refresh. (temp)") ── no storm is ever SCHEDULED, so a refresh is clear weather indefinitely.
  ...
  // reconstructing a default from memory. snowOn is already false at load, so this is the only thing that had to move: the button and P still force a storm by hand, and one turned on that way re-arms the normal 5-minute cadence on its way out. REMOVE BEFORE SHIPPING.
  snowNextT = Infinity;
