  ...
  const ED = { on: false, ret: null, prev: new Map(), frames: [], sel: 0, x0: 0, z0: 0, y: 0, pw: 242, pd: 242, fcells: [], ring: [], paused: false, animT: 0, playT0: 0, blinkE: 0, blinkT0: 0, blinkGap: 0, giz: false, dragAxis: -1, dragAcc: 0, gizBoxes: [], blink: false, spin: 0, bunny: false, rgiz: false, dragRing: -1, dragRAcc: 0, rgizBoxes: [],
    frames2: [], fcells2: [], box2: null, off2: '', name1: '', name2: '', seq1: '', seq2: '', sel2: 0, mix: [], mixI: 0, mixT0: 0, flyer2: false, spin2: 0, ex: [], hopX: 0, hopY: 0, hopZ: 0, hop2X: 0, hop2Y: 0, hop2Z: 0 };   // TWO bunnies on the stage (user): frames = LEFT/editable lane, frames2 = RIGHT/preview lane; fcells2/box2 = the preview's stamped cells + hitbox; off2 = the preview's offset namespace; name1/name2 + seq1/seq2 = which model AND which of its animations is in each lane (the export labels the lanes with both, and two animations of one model would otherwise collide under one name); sel2 = the preview's own frame index, since the two lanes' animations need not be the same length; mix/mixI/mixT0 = the editable lane's WEIGHTED PLAYLIST (one whole cycle drawn per boundary), which entry is live, and when the live cycle started; flyer2/spin2 = whether the preview lane WANDERS the stage rather than standing on it, and the cardinal heading it is facing while it does; ex = the TRACE-INJECTED exhibits (ui/editor.js edExStage), which are not lanes at all: they never touch W, so they move sub-voxel and turn freely, the way world creatures do; hop*/hop2* = per-lane forward-march accumulation (and, for a flyer, the wander's live offset)
