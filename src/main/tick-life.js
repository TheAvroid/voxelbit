    ...
    const nFish = (FISHES.length && waterSpots.length) ? Math.min(21, Math.round((6 + waterSpots.length * 4) * 0.75)) : 0;   // ── THEN A QUARTER FEWER (user 2026-09-02, later: "decrease the fish frequency by 25%") ── the whole count scaled by 0.75 and the ceiling with it (28 → 21), so the base, the per-spot growth and the cap all fall together rather than only the crowded lakes.   // ── MORE FISH (user 2026-09-02: "the fish looks sparse in the lakes. make them more abudant") ── ceiling 12 → 28, base 3 → 6, per-spot 3 → 4. The two halvings below were made when the world's water was a fraction of what it is now: the lakes are deeper, the basins wider and there is a strait between the biomes, so the same cap spread the same twelve fish over far more water and read as empty. 28 is under FISH_N (32 slots), so the band still cannot overrun its own allocation   // HALVED AGAIN (user 2026-08-18): ceiling 24→12, base 6→3, per-spot 6→3 — another uniform half off the whole curve, same as the 2026-08-09 quarter. Nothing downstream needs a minimum: FISH_N 32 was never binding, and the species split is an index not a count.   // FISH scale with how much water is in view (lakes AND rivers). CUT 25% (user 2026-08-09): ceiling 32→24, base 8→6, per-spot 8→6 — a uniform quarter off the WHOLE curve, so the cut is the same at every water amount rather than only biting where the ceiling clamped. The per-pool density cap below (~1 fish per 3 census samples) is deliberately UNTOUCHED: it is the anti-cramming ceiling, not the population, and lowering it too would thin small ponds twice. Slot band FISH_0..FISH_END is 32 wide and the live count now stops short of it; species stays an even split (wk % FISHES.length). Was DOUBLED 2026-07-21: base 4→8, per-spot 4→8, ceiling 24→32
