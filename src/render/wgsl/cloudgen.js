  ...
  const CG_CONSTS = /* wgsl */`
    ...
    const CG_NKEY : f32 = 0.40;                                      // what the deck's MOON key is scaled by at full night, in place of dayScale(). Not 1.0: sunTint's moon is already a lit-surface intensity and the deck would read as daylight
    const CG_NAMB : vec3<f32> = vec3<f32>(0.038, 0.049, 0.074);      // …and its night AMBIENT floor. Blue on purpose: a moonlit cloud is lit by the night sky, not by the moon alone
