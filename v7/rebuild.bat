...
REM is SOURCE, so it does not take effect until v7 is rebuilt -- this is the
...
echo Rebuilding v7 with the baked defaults ...
echo.
call "%~dp0build.bat" %*
set "RC=%ERRORLEVEL%"
...
echo.
if "%RC%"=="0" (
  echo Done. The baked settings are now what v7 opens with.
) else (
