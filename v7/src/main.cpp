...
void usage() {
    std::printf(
        "v7 -- voxel pine forest, path traced on Falcor (DXR inline ray tracing)\n"
        "\n"
        "  With no --out, v7 opens a window and you can walk around in it.\n"
...
bool parse(int argc, char **argv, Options *o, bool *vulkan, bool *debugLayer) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--help" || a == "-h") { usage(); return false; }
            ...
            if (i + 1 >= argc) { std::fprintf(stderr, "v7: --dlss needs a mode\n"); return false; }
            ...
            if (m == "ultra-performance") o->dlssQuality = DlssQuality::UltraPerformance;
                ...
                std::fprintf(stderr, "v7: unknown DLSS mode %s\n", m.c_str());
                return false;
            }
        }
        ...
        // be attributed to one of them instead of guessed at.
            ...
            std::fprintf(stderr, "v7: unknown option %s\n", a.c_str());
            usage();
            return false;
        }
    }
...
int main(int argc, char **argv) {
    ...
    if (o.r.width < 8 || o.r.height < 8 || o.r.spp < 1) {
        std::fprintf(stderr, "v7: nonsensical image parameters\n");
        return 1;
    }
    ...
    c.windowDesc.title = o.background ? "v7 [background] -- pine forest (Falcor)"
                                      : "v7 -- pine forest (Falcor)";
    ...
    try {
        ForestApp app(c, o);
        std::printf("v7 -- Falcor 8.0, %s, on %s\n", vulkan ? "Vulkan" : "D3D12",
