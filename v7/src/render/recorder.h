...
//
// There is no audio track: v7 has no audio. A recorder for an engine that made
...
class Nv12Convert {
  public:
    void init(Falcor::ref<Falcor::Device> device) {
        if (pass_) return;
        device_ = device;
        pass_ = ComputePass::create(device_, "v7/shaders/CaptureNv12.cs.slang", "main");
...
class Recorder {
  public:
    ...
    bool poll(Take &out) {
        ...
        std::printf("v7: recorded %s -- %dx%d @ %.3g fps, %lld frames (%.2f s)",
        ...
        std::printf("\n");
        if (!ok) std::fprintf(stderr, "v7: the recording did not produce a usable file\n");
        std::fflush(stdout);
