...
// so device and factory creation resolve to Streamline's proxies. v7 cannot do
...
//
// THAT IS WHY NOTHING HERE IS LINKED. If v7 imported sl.interposer.lib it would
...
// unavailable, and v7 falls back to the direct-NGX path in dlss.h.
// ---------------------------------------------------------------------------
#pragma once
...
class Streamline {
  public:
    ...
    static bool preInit(const std::filesystem::path &exeDir) {
        ...
        // Only the features v7 actually drives. Loading a plugin that is never
        ...
        // default because it is chatty; V7_SL_LOG=1 turns it on and points it at
        // a directory beside the build.
        const char *slLog = std::getenv("V7_SL_LOG");
    ...
    //
    // v7 traces at render resolution and presents at output resolution -- 853 x
    ...
    //
    // v7 has no matrices anywhere. Its camera is a basis and two half-extents,
    ...
    bool setFrameConstants(const float *pos, const float *right, const float *up,
        ...
        // The motion vectors v7 writes are already in RENDER PIXELS, so there
    ...
    bool tagResources(Falcor::RenderContext *ctx, Falcor::Texture *color,
        ...
        //
        // v7 has no separate UI texture: the crosshair and the settings menu are
    ...
    // How many frames the swapchain actually presented for the last one v7
