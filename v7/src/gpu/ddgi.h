...
// places, which MSVC reports as C4515 -- and v7 builds with /WX, so a warning
// in somebody else's header is a failed build. Suppressed only across these
// includes rather than switched off for the target: v7's own code should still
...
class Ddgi {
  public:
    ...
    // -----------------------------------------------------------------------
    // The same volume, described for v7's OWN two shaders -- the probe trace
  ...
  private:
    ...
    bool createTextures() {
        ...
        texRayData_ = make(T::RayData, desc_.probeRayDataFormat, "v7::ddgiRayData", rw);
        texIrradiance_ =
            make(T::Irradiance, desc_.probeIrradianceFormat, "v7::ddgiIrradiance", rwRt);
        texDistance_ = make(T::Distance, desc_.probeDistanceFormat, "v7::ddgiDistance", rwRt);
        texProbeData_ = make(T::Data, desc_.probeDataFormat, "v7::ddgiProbeData", rw);
        texVariability_ =
            make(T::Variability, desc_.probeVariabilityFormat, "v7::ddgiVariability", rw);
        texVariabilityAvg_ = make(T::VariabilityAverage, desc_.probeVariabilityFormat,
                                  "v7::ddgiVariabilityAvg", rw);
    ...
    // The two render target views Create() insists on, in a heap of v7's own.
    //
    ...
    // -----------------------------------------------------------------------
    // The descriptors the SDK's probe shaders read, written into v7's heap.
    //
