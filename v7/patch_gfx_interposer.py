...
sl.interposer.lib in place of d3d12.lib -- which v7 cannot do, because v7 does
...
REVERSIBILITY
-------------
The original is saved beside it as gfx.dll.v7-backup before anything is written,
and --restore puts it back. Nothing else in the tree is touched, so restoring
returns v7 to a stock Falcor exactly.
...
# v7 PATCHES THE STAGING COPY, NOT THE BUILD OUTPUT.
#
# v7 ships its own slang 2026.13 for cooperative vectors, staged at
# external\slang. deploycommon.bat re-copies %SlangDir%in\*.dll over the
...
BUILD = r'C:\voxelbit\v7\external\slang\bin'
GFX = os.path.join(BUILD, 'gfx.dll')
BACKUP = GFX + '.v7-backup'
...
FORWARDER = r'C:\voxelbit\v7\build\slgi\slgi.dll'
...
def restore():
    ...
    print('  v7 is back to stock Falcor (no frame generation)')
    return 0
...
if __name__ == '__main__':
    print('v7: slang-gfx / Streamline interposer patch')
