@echo off
REM ---------------------------------------------------------------------------
REM build.bat -- build the template engine against NVIDIA Falcor.
REM
...
REM   * Everything BUILT lands under template\build: template.exe, Falcor.dll, slang, the
...
REM THE DEPENDENCIES LIVE IN ANOTHER TREE, BY PATH AND NEVER BY JUNCTION -- see
REM the note at the top of CMakeLists.txt. A junction here means a recursive
REM delete of this directory walks through it and takes 680 MB of Falcor, Slang,
REM PhysX and the Agility SDK with it, which has already happened once.
...
set "WAS="
if exist "%OUT%\CMakeCache.txt" for /f "tokens=2 delims==" %%V in ('findstr /b /c:"TPL_D3D12_PREVIEW:" "%OUT%\CMakeCache.txt" 2^>nul') do set "WAS=%%V"
...
REM A running template.exe cannot be overwritten, and the failure arrives at the very
REM END of the build as a bare "LNK1104: cannot open file template.exe", which says
...
tasklist /FI "IMAGENAME eq template.exe" 2>nul | %SystemRoot%\System32\find.exe /I "template.exe" >nul
if not errorlevel 1 (
  echo.
  echo template: template.exe is still running, and the linker cannot overwrite a running exe.
...
if "%FALCOR_DIR%"=="" set "FALCOR_DIR=%DEPS%\falcor"
if not exist "%FALCOR_DIR%\CMakeLists.txt" (
  echo template: no Falcor checkout at %FALCOR_DIR%.
...
if not exist "%CMAKE%" (
  echo template: Falcor's dependencies are not fetched yet.
...
if not exist "%OUT%\CMakeCache.txt" (
  echo   cmake configure ^(Falcor + template^)
  ...
  "%CMAKE%" -S "%FALCOR_DIR%" -B "%OUT%" -G "Ninja Multi-Config" ^
    ...
    -DFALCOR_LOCAL_SLANG_DIR="%DEPS:\=/%/slang" ^
    -DFALCOR_LOCAL_SLANG_BUILD_DIR:STRING=. ^
    -DTPL_D3D12_PREVIEW=%PREVIEW%
  if errorlevel 1 ( echo template: configure failed. & exit /b 1 )
)

REM --- build ------------------------------------------------------------------
echo   ninja %CONFIG%
"%CMAKE%" --build "%OUT%" --config %CONFIG% --target template
if errorlevel 1 (
  echo.
  echo template: build failed -- nothing was changed. The previous build still runs.
  echo     If the link failed, close any running template.exe and build again.
  exit /b 1
)

echo built %OUT%\bin\%CONFIG%\template.exe
exit /b 0
