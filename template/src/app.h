...
struct Options {
    ...
    // --demo: author a test scene into the empty world. See buildDemo.
    bool demo = false;
...
class ForestApp : public SampleApp {
  public:
    ...
    void onLoad(Falcor::RenderContext *ctx) override {
        ...
        // NRC buffers only exist in a build that defined TPL_NRCSDK. Bring it up
        ...
        flycam_.warpTo(opt_.camX, opt_.camZ);
    ...
    void onFrameRender(Falcor::RenderContext *ctx, const Falcor::ref<Fbo> &target) override {
        ...
        // engine's autoPickup, at its own radius.
        {
            ...
            // NO WATERLINE TO LOOK UP. waterAt() asked the generator how high
            // the lake stood in this band. Water is mat::WATER in the volume
            // now, so "is this point submerged" is a voxel lookup the march
            // already does -- there is no global surface height, which is also
            // why two woods can no longer disagree about one.
            tracer_.waterY = -1e9f;
    ...
    void onGuiRender(Gui *pGui) override {
        ...
        if (dlss_.available()) {
            w.text(fmt("Ray Reconstruction %s", opt_.dlss ? "on" : "off"));
        } else if (!opt_.dlss) {
  ...
  private:
    ...
    bool processInput(float dt) {
        ...
        flycam_.update(move, sprint, jump, down, crouch, dt);
        pos_ = flycam_.eyePosition();
    ...
    void checkDemodulation(Falcor::RenderContext *ctx) {
        ...
        Camera cam;
        // NO GROUND TO STAND THE OFFLINE CAMERA ON. It used to be placed at
        // the world height under --cam-x/--cam-z, which needs a host-side
        // column query a store is not obliged to provide. --eye is now an
        // absolute height rather than a height above the ground; a backend that
        // can answer groundM() should put the offset back.
        cam.origin = Vec3(opt_.camX, opt_.eye, opt_.camZ);
    ...
    // -----------------------------------------------------------------------
    void makeCrosshair() {
        crosshair_ = Falcor::FullScreenPass::create(getDevice(), "template/shaders/Crosshair.ps.slang");
    ...
    void printProfile() {
        ...
        std::printf(
            "\ntemplate profile -- %zu frames at %dx%d -> %dx%d, %s\n"
    ...
    //
    // The path comes from TPL_SOURCE_DIR, baked in by the build, rather than
    ...
    std::string bakeDefaults() {
#ifndef TPL_SOURCE_DIR
        return "bake unavailable: built without TPL_SOURCE_DIR";
#else
        // TPL_SOURCE_DIR is "<engine>/src", so the engine root -- and the
        ...
        const std::string srcDir = TPL_SOURCE_DIR;
