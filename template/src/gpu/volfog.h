...
class VolFog {
  public:
    ...
    // THE STORE'S DEFINES ARE TAKEN AT INIT, NOT AT RENDER. This pass traces
    // its own shadow rays through Store.slang, so it is one of the programs
    // that has to be SPECIALISED on the backend -- exactly like the two builds
    // of Trace.cs.slang. Left out, it compiles against the default null store
    // and the fog goes blind to a world the camera can plainly see: shafts cast
    // straight through terrain, lighting the inside of a hill. That failure
    // does not name this file, or the store, or the seam.
    bool init(const Falcor::ref<Falcor::Device> &device, const Falcor::DefineList &storeDefs) {
        device_ = device;
        try {
            inject_ =
                Falcor::ComputePass::create(device_, "template/shaders/VolFogInject.cs.slang",
                                            "main", storeDefs);
        ...
        for (int c = 0; c < VolFogGrid::kCascades; ++c) {
            for (int i = 0; i < 2; ++i) {
                vol_[c][i] = device_->createTexture3D(VolFogGrid::kX, VolFogGrid::kY,
                                                      ...
                                                      nullptr, kRw);
                vol_[c][i]->setName("tpl::fogVolume");
            }
        }
    ...
    void render(Falcor::RenderContext *ctx, const V6Camera &cam, const V6Sky &sky, float density,
                float height, uint32_t frame, const Store *store, bool moving) {
        // NOT gated on the store holding anything. An empty store means every
        // shadow ray misses and the fog is uniformly lit, which is the correct
        // picture for a world with nothing in it -- and it is visible, which a
        // skipped dispatch is not.
        if (!ready_ || !store) return;
        ...
        for (int c = 0; c < VolFogGrid::kCascades; ++c) {
            ...
            // THE SAME WINDOW THE CAMERA MARCHES. This pass used to own a
            // private RayQuery against the TLAS and went blind to the ground
            // the moment the world stopped being triangles -- shafts were cast
            // straight through terrain, lighting fog inside a hill.
            store->bind(var);
