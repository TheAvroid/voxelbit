  ...
  public:
    ...
    bool init(const Falcor::ref<Falcor::Device> &device) {
        ...
        try {
            ...
            expHistogram_ =
                Falcor::ComputePass::create(device_, "template/shaders/Exposure.cs.slang", "histogram");
            expResolve_ =
                Falcor::ComputePass::create(device_, "template/shaders/Exposure.cs.slang", "resolve");
            bloomPre_ =
                Falcor::ComputePass::create(device_, "template/shaders/Bloom.cs.slang", "prefilter");
            bloomDown_ = Falcor::ComputePass::create(device_, "template/shaders/Bloom.cs.slang", "down");
            bloomUp_ = Falcor::ComputePass::create(device_, "template/shaders/Bloom.cs.slang", "up");
    ...
    void resize(int w, int h) {
        ...
        for (int i = 0; i < kBloomMaxMips; ++i) {
            ...
            auto t = device_->createTexture2D(uint32_t(lw), uint32_t(lh),
                                              ...
                                              rw);
            t->setName(("tpl::bloom" + std::to_string(i)).c_str());
            chain_.push_back(t);
            lw /= 2;
            lh /= 2;
        }
