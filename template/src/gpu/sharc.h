...
#if !TPL_HAS_SHARC
        status_ = "built without the SHaRC headers (set TPL_SHARC_DIR)";
        return false;
#else
        device_ = device;
        try {
            ...
            hashEntries_->setName("tpl::sharcHashEntries");
            lock_->setName("tpl::sharcLock");
            accum_->setName("tpl::sharcAccum");
            resolved_->setName("tpl::sharcResolved");
            ...
            stats_->setName("tpl::sharcStats");

            resolve_ =
                Falcor::ComputePass::create(device_, "template/shaders/SharcResolve.cs.slang", "main");
