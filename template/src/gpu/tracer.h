...
class Tracer {
  public:
    ...
    // decides what source the tracer is made of -- see the note on TPL_NRC in
    // Trace.cs.slang.
    void init(const Falcor::ref<Falcor::Device> &device, World *world, bool coopVec = false,
        ...
        if (coopVec_) defs.add("TPL_NRC", "1");
        ...
        if (coopVec_ && nrcVoxelFeatures_) defs.add("TPL_NRC_VOXEL_FEATURES", "1");
#if TPL_HAS_SHARC
        ...
        if (voxelKey_) defs.add("TPL_SHARC_VOXEL_KEY", "1");
#endif
#if TPL_HAS_NRCSDK
        ...
        if (nrcSdkOn_) defs.add("TPL_NRCSDK", "1");
#endif
        Falcor::ProgramDesc dt;
        dt.addShaderLibrary("template/shaders/Trace.cs.slang").csEntry("main");
        ...
        if (coopVec_) updDefs.add("TPL_NRC", "1");
        updDefs.add("TPL_SHARC_UPDATE", "1");
        if (voxelKey_) updDefs.add("TPL_SHARC_VOXEL_KEY", "1");
        Falcor::ProgramDesc du;
        du.addShaderLibrary("template/shaders/Trace.cs.slang").csEntry("sharcUpdateMain");
        if (coopVec_ && device_->getType() == Falcor::Device::Type::Vulkan)
        ...
        if (nrcSdkOn_) {
            ...
            un.add("TPL_NRCSDK_UPDATE", "1");
            Falcor::ProgramDesc du2;
            du2.addShaderLibrary("template/shaders/Trace.cs.slang").csEntry("main");
            ...
            nrcSdkResolve_ = ComputePass::create(
                device_, "template/shaders/NrcSdkResolve.cs.slang", "main", defs);
        }
#endif
        tonemap_ = ComputePass::create(device_, "template/shaders/Tonemap.cs.slang", "main");
        ...
        demodulate_ = ComputePass::create(device_, "template/shaders/Demodulate.cs.slang", "main");
        remodulate_ = ComputePass::create(device_, "template/shaders/Remodulate.cs.slang", "main");
        ...
        ddgiPlaceholder_->setName("tpl::ddgiPlaceholder");
        ...
        // looking for a difference in the picture.
        {
            ...
            blueNoiseTex_->setName("tpl::blueNoise");
        }
        ...
        if (!moonTex_) {
            ...
            moonTex_ = device_->createTexture2D(1, 1, ResourceFormat::RGBA8Unorm, 1, 1, &white,
                                                Falcor::ResourceBindFlags::ShaderResource);
            moonTex_->setName("tpl::moonFallback");
        }
        ...
        // reports an empty world, which reads as GI that simply does nothing.
        {
            ...
            probeTrace_ =
                ComputePass::create(device_, "template/shaders/ProbeTrace.cs.slang", "main", probeDefs);
        }
    }
    ...
    void resize(int renderW, int renderH, int outW, int outH) {
        ...
        accum_ = tex(ResourceFormat::RGBA32Float, w_, h_, "tpl::accum");
        color_ = tex(ResourceFormat::RGBA32Float, w_, h_, "tpl::color");
        ...
        guideAlbedo_ = tex(ResourceFormat::RGBA16Float, w_, h_, "tpl::guideAlbedo");
        guideSpecular_ = tex(ResourceFormat::RGBA16Float, w_, h_, "tpl::guideSpecular");
        guideNormRough_ = tex(ResourceFormat::RGBA16Float, w_, h_, "tpl::guideNormRough");
        guideDepth_ = tex(ResourceFormat::R32Float, w_, h_, "tpl::guideDepth");
        guideMotion_ = tex(ResourceFormat::RG16Float, w_, h_, "tpl::guideMotion");
        ...
        diffRadiance_ = tex(ResourceFormat::RGBA16Float, w_, h_, "tpl::diffRadiance");
        specRadiance_ = tex(ResourceFormat::RGBA16Float, w_, h_, "tpl::specRadiance");
        emission_ = tex(ResourceFormat::RGBA32Float, w_, h_, "tpl::emission");
        demodAlbedo_ = tex(ResourceFormat::RGBA16Float, w_, h_, "tpl::demodAlbedo");
        demodDiff_ = tex(ResourceFormat::RGBA16Float, w_, h_, "tpl::demodDiff");
        demodSpec_ = tex(ResourceFormat::RGBA16Float, w_, h_, "tpl::demodSpec");
        ...
        remodOut_ = tex(ResourceFormat::RGBA32Float, ow_, oh_, "tpl::remodOut");

        // What Ray Reconstruction writes: output resolution, still linear.
        dlssOut_ = tex(ResourceFormat::RGBA16Float, ow_, oh_, "tpl::dlssOut");
        ...
        display_ = tex(ResourceFormat::RGBA32Float, ow_, oh_, "tpl::display");
    ...
    void renderSample(Falcor::RenderContext *ctx, const V6Camera &cam, const RenderSettings &cfg) {
        ...
        //
        // Bound only when the programs were compiled with TPL_NRCSDK -- unlike the
    ...
    //
    // The program is compiled (nrcSdkUpdate_, TPL_NRCSDK_UPDATE) and its shader
    ...
    void runNrcSdkResolve(Falcor::RenderContext *ctx) {
        ...
        if (!nrcSdkScratch_ || nrcSdkScratch_->getWidth() != uint32_t(w_) ||
            ...
            nrcSdkScratch_->setName("tpl::nrcSdkScratch");
        }
