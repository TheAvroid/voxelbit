...
class Nrc {
  public:
    ...
    bool init(const Falcor::ref<Falcor::Device> &device, uint32_t maxSamples,
        ...
        if (voxelFeatures_) nd.add("TPL_NRC_VOXEL_FEATURES", "1");

        try {
            Falcor::ProgramDesc dt;
            dt.addShaderLibrary("template/shaders/NrcTrain.cs.slang").csEntry("main");
            nrcAddCapability(device_, dt);
            train_ = Falcor::ComputePass::create(device_, dt, nd);

            Falcor::ProgramDesc du;
            du.addShaderLibrary("template/shaders/NrcUpdate.cs.slang").csEntry("main");
