@echo off
REM ---------------------------------------------------------------------------
REM run.bat -- run voxelbit: the wood, path traced, with RTX neural shading
REM
...
REM
REM   run.bat              D3D12.  DLSS Ray Reconstruction, Frame Generation,
...
REM
REM   run.bat --vulkan     Vulkan. Neural shaders and the radiance cache work --
...
REM can be created at all. Plain "engine\build.bat" goes back to the stock runtime.
REM
REM ---------------------------------------------------------------------------
REM Double-click this, or run it from a prompt. Any arguments are passed
REM straight through to v1.exe:
REM
REM   run.bat                              walk around in the wood
REM   run.bat --vulkan --nrc               neural shading, with the radiance cache
REM   run.bat --seed 7 --density 0.8       a different, thicker wood
REM   run.bat --vulkan --nrc --rr 16 --depth 16
REM                                       where the cache actually PAYS -- see
REM                                       the note below
REM   run.bat --out shot.png --spp 256     render one frame offline and exit
REM   run.bat --background                 open minimised, never take focus
REM   run.bat --help                       every option
REM
...
REM            the current settings as the new defaults; engine\rebuild.bat applies
REM            them.
...
REM than hardcoded. Override it by setting ENGINE_HOME first.
REM ---------------------------------------------------------------------------
setlocal
...
REM
REM Set VOXELBIT_CONSOLE=1 before running to keep the old visible window. The
...
REM ---------------------------------------------------------------------------
if "%VOXELBIT_CONSOLE%"=="" (
  set "VOXELBIT_CONSOLE=1"
  start "voxelbit" /min cmd /c ""%~f0" %*"
  exit /b 0
)

if "%ENGINE_HOME%"=="" set "ENGINE_HOME=%~dp0engine"
set "EXE=%ENGINE_HOME%\build\bin\Release\v1.exe"

if not exist "%EXE%" (
  echo voxelbit: %EXE% not found.
  echo.
  echo     Build it first:  double-click %ENGINE_HOME%\rebuild.bat
  echo     ^(needs Visual Studio with "Desktop development with C++"; the
  echo      Falcor fork and Slang are already inside %ENGINE_HOME%\external^)
  echo.
  pause
  exit /b 1
)
...
if not exist "%ENGINE_HOME%\build\bin\Release\shaders\v1\shaders\Trace.cs.slang" (
  echo voxelbit: the shaders are missing from build\bin\Release\shaders\v1.
  echo     Rebuild with %ENGINE_HOME%\rebuild.bat
  echo.
  pause
  exit /b 1
)
...
REM v1.exe built elsewhere still finds them.
set "PINES=%~dp0game\assets\foilage\pine9"
set "DECOR=%~dp0game\assets\decoration"
if not exist "%PINES%\pine_1.vox" (
  echo voxelbit: no pine_1.vox under %PINES% -- falling back to the engine default.
  set "PINES="
)
