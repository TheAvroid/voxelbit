...
inline bool mfInit() {
    static bool ok = [] {
        ...
        if (FAILED(hr)) {
            std::fprintf(stderr, "v4: MFStartup failed (0x%08lx) -- no video recording\n",
                         (unsigned long)hr);
            return false;
        }
        return true;
...
class VideoWriter {
  public:
    ...
    bool open(const Config &cfg) {
        ...
        if (cfg_.audioChannels > 0) {
            ...
            if (audioStream_ == kNoStream)
                std::fprintf(stderr, "v4: no AAC encoder -- this take will be silent\n");
        }
    ...
    bool close() {
        ...
        if (writer_ && open_) {
            ...
            ok = SUCCEEDED(hr);
            if (!ok)
                std::fprintf(stderr, "v4: sink writer Finalize failed (0x%08lx)\n",
                             (unsigned long)hr);
        }
        writer_.Reset();
        lastBuffer_.Reset();
  ...
  private:
    ...
    bool present(IMFMediaBuffer *buf, int64_t slot) {
        ...
        if (FAILED(hr)) {
            std::fprintf(stderr, "v4: WriteSample failed (0x%08lx)\n", (unsigned long)hr);
            return false;
        }
        return true;
    }
