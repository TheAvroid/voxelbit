...
inline bool decodeToPcm48Stereo(const std::string &path, std::vector<int16_t> *out,
    ...
    if (FAILED(hr)) {
        ...
        if (!optional)
            std::fprintf(stderr, "v4: cannot open audio %s (0x%08lx)\n", path.c_str(),
                         (unsigned long)hr);
        return false;
    }
    ...
    hr = rd->SetCurrentMediaType((DWORD)MF_SOURCE_READER_FIRST_AUDIO_STREAM, nullptr,
                                 want.Get());
    if (FAILED(hr)) {
        std::fprintf(stderr, "v4: %s is not decodable to 48k stereo PCM (0x%08lx)\n",
    ...
    for (;;) {
        ...
        if (flags & MF_SOURCE_READERF_CURRENTMEDIATYPECHANGED) {
            std::fprintf(stderr, "v4: %s changes format mid-stream -- not loaded\n",
                         path.c_str());
            return false;
        }
...
class AudioDevice {
  public:
    ...
    bool open() {
        ...
        if (FAILED(hr)) {
            std::fprintf(stderr, "v4: XAudio2Create failed (0x%08lx) -- no sound\n",
                         (unsigned long)hr);
            return false;
        }
        hr = xa_->CreateMasteringVoice(&master_);
        if (FAILED(hr)) {
            std::fprintf(stderr, "v4: no audio endpoint (0x%08lx) -- no sound\n",
                         (unsigned long)hr);
            close();
            return false;
        }
        ...
        if (FAILED(master_->SetEffectChain(&chain))) {
            std::fprintf(stderr, "v4: audio tap unavailable -- recordings will be silent\n");
            delete tap_;
            tap_ = nullptr;
        }
        return true;
...
class Ambience {
  public:
    ...
    bool open(AudioDevice &dev, const std::string &path, float masterGain) {
        ...
        if (FAILED(hr)) {
            std::fprintf(stderr, "v4: CreateSourceVoice failed (0x%08lx)\n", (unsigned long)hr);
            stop();
            return false;
        }
        ...
        voice_->Start(0);
        std::printf("v4: ambience %s  (%.1f s, looping)\n", path.c_str(),
