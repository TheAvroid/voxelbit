...
// NOT <Falcor.h>, AND THAT IS DELIBERATE.
//
// The umbrella header pulls the WHOLE engine in, including the Scene system --
// Scene.h -> Volume/GridVolume.h -> Volume/Grid.h -- for a screen recorder that
// wants a texture, a buffer and a compute pass. That was merely wasteful until
// the voxel store arrived; now it is fatal. Falcor's Grid.h includes
// <nanovdb/util/GridHandle.h>, a path NanoVDB 32.7 DELETED (the header moved to
// <nanovdb/GridHandle.h>), and v2 puts its own NanoVDB ahead of Falcor's bundled
// copy on the include path -- as it must, or CreateNanoGrid.h and NanoVDB.h
// resolve to different versions. So the umbrella drags a Falcor header that
// cannot compile against the NanoVDB v2 needs, and the error names a nanovdb
// path in a Falcor file from a recorder that uses neither.
//
// Naming what is actually used costs nothing and cuts the dependency.
...
class Nv12Convert {
  public:
    void init(Falcor::ref<Falcor::Device> device) {
        if (pass_) return;
        device_ = device;
        pass_ = ComputePass::create(device_, "v4/shaders/CaptureNv12.cs.slang", "main");
...
class Recorder {
  public:
    ...
    bool start(const std::string &path, int srcW, int srcH, int maxWidth, int fpsNum, int fpsDen,
        ...
        if (audio_) {
            ...
            if ((ch == 1 || ch == 2) && (hz == 44100 || hz == 48000)) {
                ...
                std::fprintf(stderr,
                             "v4: endpoint is %d ch at %d Hz -- not something AAC takes, "
    ...
    bool poll(Take &out) {
        ...
        std::printf("v4: recorded %s -- %dx%d @ %.3g fps, %lld frames (%.2f s)",
        ...
        std::printf("\n");
        if (!ok) std::fprintf(stderr, "v4: the recording did not produce a usable file\n");
        std::fflush(stdout);
