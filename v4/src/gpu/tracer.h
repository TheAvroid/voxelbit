...
class Tracer {
  public:
    ...
    // decides what source the tracer is made of -- see the note on V4_NRC in
    // Trace.cs.slang.
    void init(const Falcor::ref<Falcor::Device> &device, World *world, bool coopVec = false,
        ...
        // WHICH GEOMETRY BACKEND THIS TRACER IS MADE OF, and it is a define
        // rather than a runtime branch on purpose. A software march that cannot
        // inline its own traversal into the ray loop is not worth measuring, so
        // the store is chosen at program-build time and specialised into every
        // pass that traces. The store supplies it -- the tracer does not know
        // the name of a single backend. See gpu/store.h.
        world_->storeDefines(defs);
        if (coopVec_) defs.add("V4_NRC", "1");
        ...
        if (coopVec_ && nrcVoxelFeatures_) defs.add("V4_NRC_VOXEL_FEATURES", "1");
#if V4_HAS_SHARC
        ...
        if (voxelKey_) defs.add("V4_SHARC_VOXEL_KEY", "1");
#endif
#if V4_HAS_NRCSDK
        ...
        if (nrcSdkOn_) defs.add("V4_NRCSDK", "1");
#endif
        Falcor::ProgramDesc dt;
        // -- SHADER EXECUTION REORDERING ---------------------------------
        //
        // Two defines and nothing else. NV_SHADER_EXTN_SLOT is the register the
        // extension UAV is declared at and MUST match what ser.h handed
        // NvAPI_D3D12_SetNvShaderExtnSlotSpace -- a disagreement does not fail,
        // it produces a pipeline whose reorder is a no-op.
        //
        // V4_SER is what Trace.cs.slang actually branches on, at COMPILE time,
        // so a build without reordering has no reorder instruction in it at all
        // rather than a runtime test around one.
        if (ser_ && ser_->ready()) {
            defs.add("V4_SER", "1");
            // NV_SHADER_EXTN_SLOT AND __SHADER_TARGET_* ARE FALCOR'S NOW.
            // With FALCOR_HAS_NVAPI on, ProgramManager adds all three as global
            // defines and Device.cpp builds the PSO extension descriptor from
            // the binding Slang actually chose. Adding them here as well was
            // how this engine ended up telling the driver one register while
            // Falcor told it another.
            //
            // THESE TWO ARE STILL OURS, because Falcor does not set them:
            //   __HLSL_VERSION          gates the whole reorder section
            //   NV_HITOBJECT_USE_MACRO_API  Slang cannot parse the templated
            //     branch -- `template <typename T> T GetAttributes()` inside
            //     NvHitObject -- and the header ships this one for that case.
            defs.add("__HLSL_VERSION", "2021");
            defs.add("NV_HITOBJECT_USE_MACRO_API", "1");
        }
        dt.addShaderLibrary("v4/shaders/Trace.cs.slang").csEntry("main");
        ...
        // -- IS THE EXTENSION PARAMETER ACTUALLY THERE? ---------------------
        //
        // Falcor decides whether to build the pipeline through NvAPI by looking
        // for a global named `g_NvidiaExt` in the program's reflection
        // (Device.cpp::findNvApiShaderParameter). If it does not find it, it
        // takes its `else` branch, builds an ORDINARY compute pipeline, and the
        // reorder then writes through a UAV nothing bound -- an access
        // violation at the first dispatch, with nothing anywhere saying why.
        //
        // So ask the same question out loud. This is the difference between
        // "reordering did not help" and "reordering was never switched on".
        if (ser_ && ser_->ready()) {
            const bool seen = trace_->getRootVar().hasMember("g_NvidiaExt");
            std::printf("  reorder  g_NvidiaExt in reflection: %s%s\n",
                        seen ? "YES" : "NO",
                        seen ? "" : "  <- Falcor will build a PLAIN pso; the reorder "
                                    "would write to an unbound UAV");
        }

#if V4_HAS_SHARC
        Falcor::DefineList updDefs;
        // THE SAME BACKEND AS THE CAMERA PASS. This is Trace.cs.slang compiled a
        // second time, and it has to shade exactly the way the camera shades or
        // the cache records a different renderer -- which includes tracing
        // against the same store. Two programs, one geometry.
        world_->storeDefines(updDefs);
        if (coopVec_) updDefs.add("V4_NRC", "1");
        updDefs.add("V4_SHARC_UPDATE", "1");
        if (voxelKey_) updDefs.add("V4_SHARC_VOXEL_KEY", "1");
        Falcor::ProgramDesc du;
        du.addShaderLibrary("v4/shaders/Trace.cs.slang").csEntry("sharcUpdateMain");
        if (coopVec_ && device_->getType() == Falcor::Device::Type::Vulkan)
        ...
        if (nrcSdkOn_) {
            ...
            un.add("V4_NRCSDK_UPDATE", "1");
            Falcor::ProgramDesc du2;
            du2.addShaderLibrary("v4/shaders/Trace.cs.slang").csEntry("main");
            ...
            nrcSdkResolve_ = ComputePass::create(
                device_, "v4/shaders/NrcSdkResolve.cs.slang", "main", defs);
        }
#endif
        ...
        demodulate_ = ComputePass::create(device_, "v4/shaders/Demodulate.cs.slang", "main");
        remodulate_ = ComputePass::create(device_, "v4/shaders/Remodulate.cs.slang", "main");
        ...
        ddgiPlaceholder_->setName("v4::ddgiPlaceholder");
        ...
        // looking for a difference in the picture.
        {
            ...
            blueNoiseTex_->setName("v4::blueNoise");
        }
        ...
        if (!moonTex_) {
            ...
            moonTex_ = device_->createTexture2D(1, 1, ResourceFormat::RGBA8Unorm, 1, 1, &white,
                                                Falcor::ResourceBindFlags::ShaderResource);
            moonTex_->setName("v4::moonFallback");
        }
        ...
        // SPECIALISED ON THE STORE like every other pass that traces. The probe
        // pass fires its own rays through Store.slang; without the backend
        // define it silently compiles against the null store and every probe
        // reports an empty world, which reads as GI that simply does nothing.
        {
            Falcor::DefineList probeDefs;
            world_->storeDefines(probeDefs);
            probeTrace_ =
                ComputePass::create(device_, "v4/shaders/ProbeTrace.cs.slang", "main", probeDefs);
        }
    }
...
#endif
    // Reordering, if the device has it. MUST be set before init(): the defines
    // it gates are read while the trace programs are built, and a pass compiled
    // without them has no reorder in it -- silently, which measures as zero.
    void setSer(Ser *s) { ser_ = s; }
    ...
    void resize(int renderW, int renderH, int outW, int outH) {
        ...
        diffRadiance_ = tex(ResourceFormat::RGBA16Float, w_, h_, "v4::diffRadiance");
        specRadiance_ = tex(ResourceFormat::RGBA16Float, w_, h_, "v4::specRadiance");
        emission_ = tex(ResourceFormat::RGBA32Float, w_, h_, "v4::emission");
        demodAlbedo_ = tex(ResourceFormat::RGBA16Float, w_, h_, "v4::demodAlbedo");
        demodDiff_ = tex(ResourceFormat::RGBA16Float, w_, h_, "v4::demodDiff");
        demodSpec_ = tex(ResourceFormat::RGBA16Float, w_, h_, "v4::demodSpec");
        ...
        remodOut_ = tex(ResourceFormat::RGBA32Float, ow_, oh_, "v4::remodOut");
    ...
    void renderSample(Falcor::RenderContext *ctx, const V6Camera &cam, const RenderSettings &cfg) {
        ...
        p.dim = dim;
        // THE WORLD, AND IT IS A VOLUME. No acceleration structure, no triangle
        // pool, no instance table -- the shader marches the NanoVDB window
        // directly. gMaterials survived all three because it was never
        // geometry: it is the palette, and the volume indexes the same table.
        var["gMaterials"] = world_->materialBuffer();
        world_->bindStore(var);
        var["gAccum"] = accum_;
        var["gColor"] = color_;
        ...
        //
        // Bound only when the programs were compiled with V4_NRCSDK -- unlike the
    ...
    void renderVolFog(Falcor::RenderContext *ctx, const V6Camera &cam, float density, float height,
                      bool moving) {
        if (!volfog_ || !volfog_->active() || !world_) return;
    ...
    //
    // The program is compiled (nrcSdkUpdate_, V4_NRCSDK_UPDATE) and its shader
    ...
    void runNrcSdkResolve(Falcor::RenderContext *ctx) {
        ...
        if (!nrcSdkScratch_ || nrcSdkScratch_->getWidth() != uint32_t(w_) ||
            ...
            nrcSdkScratch_->setName("v4::nrcSdkScratch");
        }
  ...
  private:
    ...
    // OUTSIDE THE GUARD, because its WRITE is outside the guard: renderSample
    // sets it from p.keepFilm on every frame, whether or not the NRC SDK is
    // compiled in. Declared inside, v2 does not build at all without the SDK --
    // "undeclared identifier" a thousand lines from the #if that hid it.
    bool keepFilm_ = true;
#if V4_HAS_NRCSDK
