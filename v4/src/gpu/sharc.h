...
#if !V4_HAS_SHARC
        status_ = "built without the SHaRC headers (set V4_SHARC_DIR)";
        return false;
#else
        device_ = device;
        try {
            ...
            hashEntries_->setName("v4::sharcHashEntries");
            lock_->setName("v4::sharcLock");
            accum_->setName("v4::sharcAccum");
            resolved_->setName("v4::sharcResolved");
            ...
            stats_->setName("v4::sharcStats");

            resolve_ =
                Falcor::ComputePass::create(device_, "v4/shaders/SharcResolve.cs.slang", "main");
