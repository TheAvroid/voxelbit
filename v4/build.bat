@echo off
REM ---------------------------------------------------------------------------
REM build.bat -- build v4 against NVIDIA Falcor.
REM
...
REM     v4 the top level breaks them.
...
tasklist /FI "IMAGENAME eq v4.exe" 2>nul | find /I "v4.exe" >nul
if not errorlevel 1 (
  echo.
...
if not exist "%OUT%\CMakeCache.txt" (
  ...
  REM in boost and openvdb, and v4 loads .vox files and nothing else.
