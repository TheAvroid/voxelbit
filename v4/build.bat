@echo off
REM ---------------------------------------------------------------------------
REM build.bat -- build the v4 engine against NVIDIA Falcor.
REM
...
REM   * Everything BUILT lands under v4\build: v4.exe, Falcor.dll, slang, the
...
set "WAS="
if exist "%OUT%\CMakeCache.txt" for /f "tokens=2 delims==" %%V in ('findstr /b /c:"V4_D3D12_PREVIEW:" "%OUT%\CMakeCache.txt" 2^>nul') do set "WAS=%%V"
...
REM A running v4.exe cannot be overwritten, and the failure arrives at the very
REM END of the build as a bare "LNK1104: cannot open file v4.exe", which says
...
tasklist /FI "IMAGENAME eq v4.exe" 2>nul | %SystemRoot%\System32\find.exe /I "v4.exe" >nul
if not errorlevel 1 (
  echo.
  echo v4: v4.exe is still running, and the linker cannot overwrite a running exe.
...
if not exist "%FALCOR_DIR%\CMakeLists.txt" (
  echo v4: no Falcor checkout at %FALCOR_DIR%.
...
if not exist "%CMAKE%" (
  echo v4: Falcor's dependencies are not fetched yet.
...
if not exist "%OUT%\CMakeCache.txt" (
  echo   cmake configure ^(Falcor + v4^)
  ...
  "%CMAKE%" -S "%FALCOR_DIR%" -B "%OUT%" -G "Ninja Multi-Config" ^
    ...
    -DV4_D3D12_PREVIEW=%PREVIEW%
  if errorlevel 1 ( echo v4: configure failed. & exit /b 1 )
)

REM --- build ------------------------------------------------------------------
echo   ninja %CONFIG%
"%CMAKE%" --build "%OUT%" --config %CONFIG% --target v4
if errorlevel 1 (
  echo.
  echo v4: build failed -- nothing was changed. The previous build still runs.
  echo     If the link failed, close any running v4.exe and build again.
  exit /b 1
)

echo built %OUT%\bin\%CONFIG%\v4.exe
exit /b 0
