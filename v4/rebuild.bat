...
REM is SOURCE, so it does not take effect until v4 is rebuilt -- this is the
...
echo Rebuilding v4 with the baked defaults ...
echo.
call "%~dp0build.bat" %*
set "RC=%ERRORLEVEL%"
...
echo.
if "%RC%"=="0" (
  echo Done. The baked settings are now what v4 opens with.
) else (
