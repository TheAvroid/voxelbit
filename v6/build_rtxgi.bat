@echo off
REM ---------------------------------------------------------------------------
REM build_rtxgi.bat -- build the RTXGI DDGI static library v6 links against.
REM
REM Run once. The result is a .lib plus the SDK headers, and v6's CMakeLists
...
REM RIGHT HAND, Y-UP is not a preference -- it is v6's world. The terrain is a
...
REM
REM RESOURCE_MANAGEMENT=OFF puts the SDK in UNMANAGED mode: v6 creates the probe
...
REM Managed mode the SDK creates the probe textures, and v6's own Slang shaders
