...
REM
REM UNMANAGED Resource Mode is why the REGS list below exists at all. v6 creates
REM the probe textures itself -- so that Falcor owns them and its barrier
REM tracking stays correct across v6 own Slang probe passes -- and the price is
...
set "OUT=%~1"
if "%OUT%"=="" set "OUT=%~dp0build\bin\Release\shaders\v6\ddgi"
...
REM RTXGI_COORDINATE_SYSTEM=2 is RIGHT HAND, Y-UP, and it is v6's world rather
