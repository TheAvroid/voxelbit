...
set "OUT=%HERE%\build\bin\Release"
...
if errorlevel 1 ( echo build_slgi: could not install slgi.dll ^(is v6.exe running?^) & exit /b 1 )
copy /y "%TMPDIR%\slgi.dll" "%OUT%\slp12.dll" >nul
if errorlevel 1 ( echo build_slgi: could not install slp12.dll ^(is v6.exe running?^) & exit /b 1 )
