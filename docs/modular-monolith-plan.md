...
**Status: Phase A is DONE (2026-08-09).** `game/index.html` (16,226 lines / 1.52 MB) is
now generated by `tools/bundle.py` from 78 ordered fragments in `src/`, and the rebuild
is byte-identical — `git diff` on the artifact is empty. Phase C's linter shipped early,
as `tools/lint-vb.py`. **Phase B is part-done (2026-08-09):** B5 (shader factories) is
shipped and verified; B4 and B7 are delivered as machine-checked invariants rather than
module extractions — see §4. The ES-module conversion itself (B1–B3, B6, B8–B15) is NOT
started as an `import`/`export` conversion — see the measurement below for why, and what
was built instead.

**Measured 2026-08-09, and it changes the shape of the rest of Phase B:** of 334 top-level
`let`s, **211 are assigned from a fragment other than the one that declares them**. Under
real ESM you cannot assign to an imported binding at all, so a wholesale conversion is not
15 mechanical steps — it is 211 pieces of shared mutable state to restructure, in a game
that is CPU-bound at 3.7 ms a frame. That is the plan's own "hot-state rule", and it is a
much bigger number than the plan assumed (it says 191 hot `let`s, but does not say how many
cross fragment boundaries).

So the per-file scope was bought a different way: `bundle.py` gives any fragment marked
`// @module` its own IIFE scope, returning exactly its `// @exports` list. That delivers the
one thing full ESM was for — private names that cannot collide — incrementally, per file,
with no change to how anything is accessed and no restructuring of shared state. Check 10
derives the correct export list and refuses a module that would export a cross-written
`let`. Three fragments are converted (`ui/console.js`, `ui/editor.js`, `ui/video-editor.js`),
taking 105 names out of the shared scope; the remaining 75 can follow one at a time,
whenever a fragment's private surface makes it worth doing.

The map of what ended up in which fragment is [architecture.md](architecture.md); the
working rules are in [CLAUDE.md](../CLAUDE.md). What follows is the original plan, kept
because sections 4–7 are still the plan for Phase B.

**Target:** `game/index.html` — one `<style>` block and one `<script type="module">`
containing a single `(async () => { ... })()` IIFE.
...
## 3. Phase A — mechanical split, provably behaviour-identical — **DONE**
...
- 78 fragments, not ~30 — the extra cuts are the twelve WGSL shaders taken separately,
  `window.__vb` on its own (it is 1,031 lines), and `tickBody` split into seven, since
  otherwise the frame loop would have been one 2,600-line file that no two agents could
  touch at once.
- The `<style>` block was split too (7 CSS fragments), which the plan did not ask for
  until B1. It was free: the same byte-identical gate covers it.
- **Dev needs no build step.** `tools/serve-nocache.py` concatenates `src/` in memory on
  every request for `/`, so edit-and-refresh works exactly as it did with the single
  file. `tools/bundle.py` writes the artifact for commit and release.
- The gate held: `python tools/bundle.py --check` reports the rebuild identical at
  1,556,647 bytes, and `git diff --exit-code -- game/index.html` is clean.
...
### B4 — the uniform struct — **DONE, as a build-time check**

Built as check 8 of `lint-vb.py` rather than a runtime-derived field list. The plan's goal
was to turn "append-only or else" into something the machine enforces; the check computes
the real WGSL layout from `struct U` and fails the build naming exactly which offsets
moved. That reaches the same guarantee at **zero runtime cost**, which matters because the
game is CPU-bound and the standing rule is that a refactor must be perceptually lossless.

The layout computation was validated before it was trusted: it reproduces all 17 offsets
documented in the comments (`drops` 68, `fflies` 1108, `physB` 1532, `heldCfg` 1860,
`dropsB` 1876, `dof` 3156, …) and the exact total of 3160 floats. It was then verified to
FIRE, by inserting a field mid-struct (16 errors, each naming a shifted field) and by
appending one without growing `UF` (caught as a length mismatch).
...
### B5 — shaders become factories — **DONE**

All 12 WGSL literals are now `*_SRC` factories, called from one SHADER BUILD block in
`render/wgsl/vis.js` just above the pipelines. Shader-to-shader dependencies (`DDAW`,
`FLAKEBLK`, `pickWGSL`) are passed explicitly, and because the destructured parameter
names shadow the outer ones, **every shader body is byte-for-byte unchanged**.

Verified rather than assumed: a temporary probe hashed all 12 compiled WGSL strings before
and after — 12 of 12 identical, to the character. Then the property itself was tested by
making `PRE` interpolate a const declared five fragments later; it boots clean, where
before it was a ReferenceError at module-eval and a black screen.
...
### B7 — the worker contract — **DONE, as a build-time check**

Built as check 9 of `lint-vb.py`. It extracts each of the 52 serialized functions, strips
its locals and parameters, and flags any identifier that is a **top-level game name** but
is not in the `consts` / `tables` / `fns` registries or the worker preamble. Scoping it to
provably-top-level names is what keeps it precise: a local, a parameter or `Math` cannot
reach the report, so there are no false positives to argue with — it runs clean on all 52
functions today, and fires on both an unregistered call and an unregistered table read.

This does not require `gen-core.js` to exist as a module. The contract is enforced where
the functions actually live.
...
## 5. Phase C — make the boundaries hold — **shipped early, adapted**

`tools/lint-vb.py` exists and runs clean. It landed with Phase A rather than after Phase
B, because the fragment layout creates one failure mode the single file could not have —
a duplicate top-level `const` in two files nobody is reading side by side — and that
needed catching immediately.

The seven checks as built: manifest integrity, byte hygiene, brace balance (the mid-line
`//` boot-killer), fragment boundaries landing in ordinary code, **duplicate top-level
declarations across the whole shared scope**, backticks in WGSL comments, and a stale
`game/index.html`. Each was verified to actually fire by breaking a scratch copy of
`src/` on purpose — a check that never fires is worthless.

Note that the plan below expected duplicate top-level `const` to *stop being possible*
once each module had its own scope. That only becomes true in Phase B; until then it is
check 5, and it is the most valuable check in the file.

The two layering checks (1 and 2 below) are **not** built — there are no imports yet to
point sideways or upward. They belong with Phase B.
